...
import {
	...
	type SerializedTeam,
	STORAGE_KEY,
	type TeamSetSavedCookingSettings,
	type TeamSetSavedFieldSettings,
	type TeamSetSimulationSnapshot,
	type TeamSetState,
	type TeamTimelineAction,
	type TeamTimelineState,
...
export const STORAGE_KEY_LEFTOVER_INCLUDE_EXTRA_USAGE =
	"PstTeamTimelineLeftoverIncludeExtraUsage";
...
const VALID_COOKING_CATEGORIES: readonly CookingCategory[] = [
	"curry",
	"salad",
	"dessert",
];

function cloneFavoriteType(
	value: TimelineFavoriteTypes,
): TimelineFavoriteTypes {
...
function clampActiveTeamSetIndex(
	index: number,
	teamSets: readonly TeamSetState[],
): number {
...
function replaceActiveTeamSet(
	...
	const activeIndex = clampActiveTeamSetIndex(
...
function getSwapPosition(
	swap: Pick<PokemonSwap, "dayIndex" | "slotId" | "teamSlotIndex">,
): SwapPosition {
	return {
...
export function teamTimelineReducer(
	...
	switch (action.type) {
		case "renameActiveTeamSet": {
			...
			return {
				...
				teamSets: state.teamSets.map((set, index) =>
					index === activeIndex ? { ...set, name: action.name } : set,
				),
			};
		}
		...
		case "duplicateTeamSet": {
			...
			const sourceSet =
				state.teamSets[activeIndex] ??
				createEmptyTeamSet(action.id, action.name);
		...
		case "deleteTeamSet": {
			if (state.teamSets.length <= 1) {
				const replacement = createEmptyTeamSet(
					action.fallbackId,
					action.fallbackName,
				);
				return {
			...
			const nextTeamSets = state.teamSets.filter(
				(_, index) => index !== activeIndex,
			);
			const nextActiveIndex = clampActiveTeamSetIndex(
				activeIndex,
				nextTeamSets,
			);
		...
		case "loadTeamSets": {
			const normalizedTeamSets =
				action.teamSets.length > 0
		...
		case "updateSimulationConfig": {
			...
			if (action.config.simulationDays !== undefined) {
				mergedConfig.simulationDays = clampSimulationDays(
		...
		case "confirmSwap": {
			...
			return replaceActiveTeamSet(
				nextState,
				state.team,
				[...swapsWithoutOldRepeats, ...allNewSwaps],
				state.noCollectCells,
			);
		}
		case "confirmSwapDirect": {
			...
			return replaceActiveTeamSet(
				nextState,
				state.team,
				[...filteredSwaps, newSwap],
				state.noCollectCells,
			);
		}
		case "removeSwap": {
			const newSwaps = state.swaps.filter((swap) => {
		...
		case "moveSwapSeries": {
			...
			const sourceSeries = collectRepeatSeriesFromSwap(
				state.swaps,
				sourceAnchor,
			);
			...
			let remainingSwaps = state.swaps.filter(
				(swap) =>
					!sourceSeriesKeySet.has(toSwapPositionKey(getSwapPosition(swap))),
			);

			const targetAnchor = findSwapAtPosition(remainingSwaps, targetPosition);
			if (targetAnchor) {
				const targetSeries = collectRepeatSeriesFromSwap(
					remainingSwaps,
					targetAnchor,
				);
				...
				remainingSwaps = remainingSwaps.filter(
					(swap) =>
						!targetSeriesKeySet.has(toSwapPositionKey(getSwapPosition(swap))),
				);
			}
			...
			const movedSwaps = sourceSeries
				.map((swap): PokemonSwap | null => {
					...
					if (
						movedDayIndex < 0 ||
						movedDayIndex >= state.simulationConfig.simulationDays
					) {
						return null;
					}
					return {
			...
			remainingSwaps = remainingSwaps.filter(
				(swap) =>
					!movedSwapKeySet.has(toSwapPositionKey(getSwapPosition(swap))),
			);

			return replaceActiveTeamSet(
				state,
				state.team,
				[...remainingSwaps, ...movedSwaps],
				state.noCollectCells,
			);
		}
		case "clearSwaps":
		...
		case "toggleNoCollectCell": {
			...
			return replaceActiveTeamSet(state, state.team, state.swaps, [
				...state.noCollectCells,
				nextTarget,
			]);
		}
		...
		case "setBonusSettings": {
			...
			const nextTeamSets = state.teamSets.map((set, index) => {
				...
				return {
					...set,
					savedFieldSettings: createSavedFieldSettingsFromState(
						action.settings,
					),
				};
			});
		...
		case "setCookingSettings": {
			...
			const nextTeamSets = state.teamSets.map((set, index) => {
				...
				return {
					...set,
					savedCookingSettings: createSavedCookingSettingsFromState(
						action.settings,
					),
				};
			});
 ...
 */
export function loadTeamFromStorage(
	box: PokemonBox,
): (PokemonBoxItem | null)[] {
...
function migratePokemonSwap(rawSwap: unknown): PokemonSwap | null {
	...
	return {
		...
		newPokemonSerialized:
			typeof candidate.newPokemonSerialized === "string"
...
function migrateTeamSetSimulationSnapshot(
	rawSnapshot: unknown,
): TeamSetSimulationSnapshot | null {
...
function migrateTeamSetSavedCookingSettings(
	rawSettings: unknown,
): TeamSetSavedCookingSettings | null {
...
function migrateTeamSetSavedFieldSettings(
	rawSettings: unknown,
): TeamSetSavedFieldSettings | null {
	if (!rawSettings || typeof rawSettings !== "object") {
		return null;
	}
	...
	const normalized = normalizeTimelineBonusSettings({
		fieldIndex:
			typeof candidate.fieldIndex === "number" &&
			Number.isFinite(candidate.fieldIndex)
...
function serializeTeam(
	...
	return normalizeTeamLength(team).map((item) =>
		item ? item.serialize() : null,
	);
}

export function saveTeamSetsToStorage(
		...
		teamSets: teamSets.map((teamSet) => ({
			...
			savedFieldSettings: teamSet.savedFieldSettings
				? {
						...
						favoriteType: cloneFavoriteType(
							teamSet.savedFieldSettings.favoriteType,
						),
					}
				: null,
		})),
	...
	try {
		...
		const teamSets = parsed.teamSets
			.map((rawTeamSet, index): TeamSetState | null => {
				...
				const id =
					typeof rawTeamSet.id === "string" && rawTeamSet.id.length > 0
						? rawTeamSet.id
						: `team-set-${index + 1}`;
				const name =
					typeof rawTeamSet.name === "string" && rawTeamSet.name.length > 0
						? rawTeamSet.name
						: `Team ${index + 1}`;
				...
				const noCollectCells = Array.isArray(rawTeamSet.noCollectCells)
					...
					: [];
				const lastSimulationSnapshot = migrateTeamSetSimulationSnapshot(
					rawTeamSet.lastSimulationSnapshot,
				);
				const saveCookingSettings = rawTeamSet.saveCookingSettings === true;
				const saveFieldSettings = rawTeamSet.saveFieldSettings === true;
				const migratedSavedCookingSettings = migrateTeamSetSavedCookingSettings(
					rawTeamSet.savedCookingSettings,
				);
				const migratedSavedFieldSettings = migrateTeamSetSavedFieldSettings(
					rawTeamSet.savedFieldSettings,
				);
				return {
					id,
					name,
					...
					savedFieldSettings: saveFieldSettings
						? (migratedSavedFieldSettings ?? {
								...
								favoriteType: cloneFavoriteType(
									DEFAULT_TIMELINE_BONUS_SETTINGS.favoriteType,
								),
							})
						: null,
				};
		...
		const activeTeamSetIndex = clampActiveTeamSetIndex(
			parsed.activeTeamSetIndex ?? 0,
			teamSets,
		);
 ...
 */
export function saveBonusSettingsToStorage(
