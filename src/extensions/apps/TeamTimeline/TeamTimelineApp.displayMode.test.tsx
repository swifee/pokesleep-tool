...
vi.mock("./components/ResimulationNoticeBar", () => ({
	default: ({
		...
		>
			<button
				type="button"
				data-testid="resimulation-notice-run"
			...
			<button
				type="button"
				data-testid="resimulation-notice-undo"
				onClick={() => onUndo?.()}
			>
				undo-resimulation
			</button>
			<button
				type="button"
				data-testid="resimulation-notice-close"
				onClick={() => onClose?.()}
			>
				close-resimulation
			</button>
...
describe("TeamTimelineApp timeline display mode", () => {
	...
	it("uses detailed mode by default and toggles to simple mode", () => {
		...
		expect(
			screen
				.getByTestId("team-timeline-post-simulation-scroll-container")
				.getAttribute("data-scroll-overflow-x"),
		).toBe("auto");
	...
	it("does not persist display mode after remount", () => {
		...
		fireEvent.click(screen.getByRole("switch", { name: "シンプル表示" }));
		expect(
			screen.getByTestId("timeline-table").getAttribute("data-display-mode"),
		).toBe("simple");
	...
	it("opens team box dialog when clicking header slot after simulation", () => {
		...
		expect(
			screen.getByTestId("team-box-dialog").getAttribute("data-open"),
		).toBe("false");
	...
	it("keeps simulation details and shows re-simulation notice when team member changes", () => {
		...
		expect(
			screen.getByTestId("resimulation-notice").getAttribute("data-open"),
		).toBe("false");
	...
	it("persists no-collect cells and restores them after remount", () => {
		...
		expect(
			screen
				...
				.getAttribute("data-no-collect-count"),
		).toBe("0");
		...
		if (teamSetsPayload === null) {
			throw new Error("Expected team sets payload to be persisted");
		}
	...
	it("shows result diff after re-simulation and restores previous settings on undo", async () => {
		...
		expect(
			screen.getByTestId("timeline-table").getAttribute("data-team-member-0"),
		).toBe("null");
		...
		await waitFor(() => {
			expect(
				screen.getByTestId("resimulation-notice").getAttribute("data-mode"),
			).toBe("result");
		});
		expect(
			screen
				.getByTestId("resimulation-notice")
				.getAttribute("data-total-delta"),
		).toBe("10000");
	...
	it("shares leftover toggle state across average and details summary rows", () => {
		...
		expect(
			screen.getByTestId("leftover-toggle-state-average").textContent,
		).toBe("off");
		expect(
			screen.getByTestId("leftover-toggle-state-details").textContent,
		).toBe("off");
	...
	it("persists leftover toggle state across remounts", () => {
		...
		expect(
			localStorage.getItem("PstTeamTimelineLeftoverIncludeExtraUsage"),
		).toBe("1");
	...
	it("keeps simulation seed unchanged when selecting trial from simulation details slider", () => {
		...
		expect(screen.getByTestId("simulation-controls-seed").textContent).toBe(
			"123456",
		);
