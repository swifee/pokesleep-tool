...
describe("AnalysisBaseMetricsUtils", () => {
	it("returns cached metrics when quick mode matches cache mode", () => {
		...
		const simulationMetrics = createMetrics({
			teamEP: 9999,
			teamHelpCount: 999,
		});
		...
		const resolved = resolvePrecomputedBaseAverageMetrics(
			cache,
			true,
			simulationMetrics,
		);
	...
	it("ignores simulation metrics in quick mode when cache is not available", () => {
		...
		const resolved = resolvePrecomputedBaseAverageMetrics<TestMetrics>(
			null,
			true,
			simulationMetrics,
		);
	...
	it("falls back to simulation metrics in normal mode when cache mode differs", () => {
		const cachedQuickMetrics = createMetrics({
			teamEP: 4000,
			teamHelpCount: 200,
		});
		const simulationMetrics = createMetrics({
