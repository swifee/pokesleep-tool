...
function toFavoriteTypeArray(
	value: unknown,
	fallback: TimelineFavoriteTypes,
): TimelineFavoriteTypes {
...
function fillMissingFavoriteTypes(
	types: (PokemonType | null | undefined)[],
): TimelineFavoriteTypes {
	const next = [...types];
	...
	const first = next[0] ?? "normal";
	const second = next[1] ?? "normal";
	const third = next[2] ?? "normal";
	return [first, second, third];
}

function normalizeFavoriteTypesForEvent(
	...
	const fixedBerries = (eventBonus?.fixedBerries ?? []) as (
		| PokemonType
		| null
		| undefined
	)[];
	...
	for (let i = 0; i < 3; i += 1) {
		if (
			fixedBerries[i] &&
			PokemonTypes.includes(fixedBerries[i] as PokemonType)
		) {
 ...
 */
export function strengthParameterToTimelineBonusSettings(
...
export function normalizeTimelineBonusSettings(
	...
	const normalized: TimelineBonusSettings = {
		fieldIndex: Number.isInteger(input.fieldIndex)
			? Math.max(
					MIN_FIELD_INDEX,
					Math.min(MAX_FIELD_INDEX, input.fieldIndex as number),
				)
			: DEFAULT_BONUS_SETTINGS.fieldIndex,
		favoriteType: toFavoriteTypeArray(
			input.favoriteType,
			DEFAULT_BONUS_SETTINGS.favoriteType,
		),
		expertEffect: isExpertEffect(input.expertEffect)
			? input.expertEffect
			: DEFAULT_BONUS_SETTINGS.expertEffect,
		fieldBonus:
			typeof input.fieldBonus === "number"
				? Math.min(100, Math.max(0, Math.floor(input.fieldBonus)))
				: DEFAULT_BONUS_SETTINGS.fieldBonus,
		isGoodCampTicketSet:
			typeof input.isGoodCampTicketSet === "boolean"
				? input.isGoodCampTicketSet
				: DEFAULT_BONUS_SETTINGS.isGoodCampTicketSet,
		event:
			typeof input.event === "string"
				? input.event
				: DEFAULT_BONUS_SETTINGS.event,
		customEventBonus: loadHelpEventBonus(input.customEventBonus),
		recipeBonus:
			typeof input.recipeBonus === "number" &&
			RECIPE_BONUS_OPTIONS.has(input.recipeBonus)
				? input.recipeBonus
 ...
 */
export function saveTimelineBonusSettingsToIvStorage(
	settings: TimelineBonusSettings,
): StrengthParameter {
	...
	localStorage.setItem(
		...
		JSON.stringify(mergedParameter),
	);
