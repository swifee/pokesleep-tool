...
export function sortIngredientsByCountDesc<T extends IngredientResult>(
...
export function filterPositiveIngredients<T extends IngredientResult>(
	ingredients: readonly T[],
): T[] {
	return ingredients.filter((ingredient) => ingredient.count > 0);
}

export function calculateIngredientTotalCount(
...
export function groupLowDailyIngredientsForAverage(
	...
	const sortedIngredients = sortIngredientsByCountDesc(
		filterPositiveIngredients(ingredients),
	);
