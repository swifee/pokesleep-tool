...
describe('groupAverageCookingRecipes', () => {
    it('keeps >=1 as visible, groups <1, and excludes <0.01', () => {
        const grouped = groupAverageCookingRecipes([
            { recipeName: 'visible', eBase: 100, averageCount: 1, averageCookingEP: 1000 },
            { recipeName: 'grouped', eBase: 200, averageCount: 0.5, averageCookingEP: 500 },
            { recipeName: 'excluded', eBase: 300, averageCount: 0.009, averageCookingEP: 100 },
        ]);

        expect(grouped.visibleRecipes.map(recipe => recipe.recipeName)).toEqual(['visible']);
        expect(grouped.groupedRecipes.map(recipe => recipe.recipeName)).toEqual(['grouped']);
