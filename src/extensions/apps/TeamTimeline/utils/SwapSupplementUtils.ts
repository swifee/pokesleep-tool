...
import { TimelineDurationSummary } from './AdditionalAnalysisUtils';
...
export function buildSwapSupplementSequences(input: BuildSwapSupplementSequencesInput): SwapSupplementSequence[] {
    ...
    const slotOrderById = new Map<string, number>(
        buildExpandedTimeline([...timeSlots], 1).baseDaySlots.map((slot, index) => [slot.id, index] as const)
    );
    ...
    [...swapsByTeamSlot.entries()]
        ...
        .forEach(([teamSlotIndex, teamSlotSwaps]) => {
            const sortedSwaps = [...teamSlotSwaps].sort((left, right) => {
                ...
                const leftSlotOrder = slotOrderById.get(left.slotId) ?? Number.MAX_SAFE_INTEGER;
                const rightSlotOrder = slotOrderById.get(right.slotId) ?? Number.MAX_SAFE_INTEGER;
            ...
            const appendPokemonId = (pokemonId: number | null | undefined): void => {
                if (pokemonId === null || pokemonId === undefined || pokemonId === SWAP_NONE_POKEMON_ID) {
                    return;
                }
            ...
            sortedSwaps.forEach((swap) => appendPokemonId(swap.newPokemonId));
            ...
            const entries = orderedPokemonIds
                .map((pokemonId): SwapSupplementPokemonEntry | null => {
                    ...
                    const activeMinutes = durationSummary.activeMinutesByPokemonId.get(pokemonId) ?? 0;
                    const activeRatioPercent = totalTimelineMinutes > 0
