...
describe("FirstAccessPreset", () => {
	...
	it("seeds first access team and swaps when related storages are empty", () => {
		...
		if (payloadRaw === null) {
			throw new Error("Expected team set payload to exist");
		}
	...
	it("does not overwrite existing team storage", () => {
		localStorage.setItem(
			...
			JSON.stringify({ activeTeamSetIndex: 0, teamSets: [] }),
		);

		applyFirstAccessPresetIfNeeded();

		const storedTeamSets = localStorage.getItem(STORAGE_KEY_TEAM_SETS);
		if (storedTeamSets === null) {
			throw new Error("Expected existing team storage to remain available");
		}
		expect(JSON.parse(storedTeamSets)).toEqual({
	...
	it("builds runtime box with user entries + hidden preset entries", () => {
		const userItem = new PokemonBoxItem(
			PokemonIv.deserialize(PIKACHU_SERIALIZED),
		);
