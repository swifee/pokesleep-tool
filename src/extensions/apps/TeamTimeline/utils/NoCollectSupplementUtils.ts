...
export function countActiveNoCollectCells(
	...
	return noCollectCells.filter(
		(cell) =>
			!swapCellKeySet.has(
				toCellKey(cell.dayIndex, cell.slotId, cell.teamSlotIndex),
			),
	).length;
}

export function buildNoCollectSupplementEntries(
	input: BuildNoCollectSupplementEntriesInput,
): NoCollectSupplementEntry[] {
	const { team, swaps, noCollectCells, timeSlots, simulationDays, box } = input;
	...
	const currentTeamBySlot = team.map(
		(member) => member?.id ?? SWAP_NONE_POKEMON_ID,
	);
	...
	expandedTimeline.expandedSlots.forEach((expandedSlot) => {
		...
		slotSwaps.forEach((swap) => {
			if (
				swap.teamSlotIndex < 0 ||
				swap.teamSlotIndex >= currentTeamBySlot.length
			) {
				return;
			}
		...
		[...slotNoCollectCells]
			...
			.forEach((cell) => {
				if (
					cell.teamSlotIndex < 0 ||
					cell.teamSlotIndex >= currentTeamBySlot.length
				) {
					return;
				}
				...
				countByPokemonId.set(
					pokemonId,
					(countByPokemonId.get(pokemonId) ?? 0) + 1,
				);
			});
	});
