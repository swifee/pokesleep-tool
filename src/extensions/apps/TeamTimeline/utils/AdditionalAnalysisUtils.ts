...
const NIGHTMARE_SKILLS = new Set<string>(["Charge Strength M (Bad Dreams)"]);
...
export function hasAnyPlusOrMinusSkill(
	members: readonly PokemonBoxItem[],
): boolean {
	return members.some((member) =>
		PLUS_MINUS_SKILLS.has(getEffectiveMainSkillName(member)),
	);
}
...
function collectCookingMinusEligibleIdsByTimeline(
	...
	const inspectCurrentTeam = (): void => {
		const plusMinusMembers = currentTeam.filter(
			(member): member is PokemonBoxItem =>
				member !== null &&
				isPlusOrMinusSkill(getEffectiveMainSkillName(member)),
		);
...
export function getTeamHelpCount(result: SimulationResult): number {
	return result.dailySummaries.reduce(
		(sum, summary) => sum + summary.totalHelpCount,
		0,
	);
}

export function getPokemonHelpCount(
	result: SimulationResult,
	pokemonId: number,
): number {
	return (
		result.dailySummaries.find((summary) => summary.pokemonId === pokemonId)
			?.totalHelpCount ?? 0
	);
}

export function getPokemonTotalEP(
	result: SimulationResult,
	pokemonId: number,
): number {
	return (
...
function hasEnergyRecoveryBonusSubSkill(member: PokemonBoxItem): boolean {
	return member.iv.activeSubSkills.some(
		(subSkill) => subSkill.name === ENERGY_RECOVERY_BONUS_SUBSKILL,
	);
}

function hasHelpingBonusSubSkill(member: PokemonBoxItem): boolean {
	return member.iv.activeSubSkills.some(
		(subSkill) => subSkill.name === HELPING_BONUS_SUBSKILL,
	);
}
...
export function collectWakeErbMemberCountRange(
	...
	expandedTimeline.expandedSlots.forEach((expandedSlot) => {
		if (getDisplayLabel(expandedSlot.slot) === "wake") {
			const erbCount = currentTeam.filter(
				(member): member is PokemonBoxItem =>
...
export function collectAverageHelpingBonusMemberCountByDuration(
	...
	expandedTimeline.expandedSlots.forEach((expandedSlot, index) => {
		const prevSlot =
			index > 0 ? expandedTimeline.expandedSlots[index - 1].slot : null;
		const durationMinutes = prevSlot
			? calculateDuration(prevSlot.time, expandedSlot.slot.time)
			: 0;
		const helpingBonusMemberCount = currentTeam.filter(
			(member): member is PokemonBoxItem =>
				member !== null && hasHelpingBonusSubSkill(member),
		).length;
		weightedHelpingBonusMemberMinutes +=
			helpingBonusMemberCount * durationMinutes;
	...
	expandedTimeline.expandedSlots.forEach((expandedSlot) => {
		...
		if (label === "sleep") {
			...
			const sleepMinutes = calculateDuration(
				latestSleepSlot.time,
				expandedSlot.slot.time,
			);
			const erbMemberCount = currentTeam.filter(
				(member): member is PokemonBoxItem =>
					member !== null && hasEnergyRecoveryBonusSubSkill(member),
			).length;
...
export function buildEnergySkillContributionResult(
	...
	const scenarioSelfHelpCount = getPokemonHelpCount(
		scenarioResult,
		target.pokemonId,
	);
