import { TimeSlot, TimeSlotLabel } from '../types/TimeSlotTypes';
 ...
 */
export function isSleepingSlot(slot: TimeSlot, slots: TimeSlot[], isFirstSlot: boolean = false): boolean {
...
export function sortTimeSlots(slots: TimeSlot[]): TimeSlot[] {
    ...
    const withRelativeTime = slots.map((slot) => {
        ...
        const relativeTime = time >= DAY_START_MINUTES
