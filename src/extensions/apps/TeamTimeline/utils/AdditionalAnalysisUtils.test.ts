...
import type { EnergySkillContributionTarget } from "../types/AdditionalAnalysisTypes";
import {
	type PokemonSwap,
...
function createMockResult(
	teamEP: number,
	helpByPokemonId: ReadonlyMap<number, number>,
): SimulationResult {
	return {
		slotResults: new Map(),
		dailySummaries: Array.from(helpByPokemonId.entries()).map(
			([pokemonId, totalHelpCount]) => ({
				pokemonId,
				totalHelpCount,
				totalSkillCount: 0,
				totalBerryCount: 0,
...
describe("AdditionalAnalysisUtils", () => {
	...
	it("builds energy skill targets with category and conditional Cooking Minus inclusion", () => {
		...
		const targetsWithoutPlusMinus = buildEnergySkillContributionTargets([
			selfMember,
			teamMember,
		]);
		expect(
			targetsWithoutPlusMinus.some(
				(target) => target.pokemonId === minusMember.id,
			),
		).toBe(false);
		...
		const byId = new Map<number, EnergySkillContributionTarget>(
			targets.map((target) => [target.pokemonId, target]),
		);
	...
	it("shows Cooking Minus target only when timeline has concurrent plus/minus teammate timing", () => {
		...
		expect(
			targetsWithConcurrentTiming.some(
				(target) => target.pokemonId === minusMember.id,
			),
		).toBe(true);
		...
		expect(
			targetsWithoutConcurrentTiming.some(
				(target) => target.pokemonId === minusMember.id,
			),
		).toBe(false);
	});
	...
	it("shows Cooking Minus targets when multiple minus members are simultaneously active", () => {
		...
		const targets = buildEnergySkillContributionTargets([minusA, minusB], {
	...
	it("builds contribution/energy/erb analysis result payloads", () => {
		...
		const scenario = createMockResult(
			9200,
			new Map([
				[member.id, 80],
				[999, 180],
		...
		const contribution = buildContributionEpAnalysisResult(
			member,
			base,
			scenario,
		);
		...
		const energySkill = buildEnergySkillContributionResult(
			target,
			base,
			scenario,
		);
