...
function createIngredientMagnetPresentPokemon(skillLevel: number): PokemonBoxItem {
...
function createIngredientDrawSuperLuckPokemon(skillLevel: number): PokemonBoxItem {
    return createPokemonBySkill('Ingredient Draw S (Super Luck)', skillLevel);
}

function createIngredientDrawHyperCutterPokemon(skillLevel: number): PokemonBoxItem {
...
function createLunarBlessingPokemon(skillLevel: number): PokemonBoxItem {
    return createPokemonBySkill('Energy for Everyone S (Lunar Blessing)', skillLevel);
}

function createBerryJuicePokemon(skillLevel: number): PokemonBoxItem {
    return createPokemonBySkill('Energy for Everyone S (Berry Juice)', skillLevel);
}
...
function createDreamShardMagnetRandomPokemon(skillLevel: number): PokemonBoxItem {
...
function totalIngredientCount(ingredients: { name: string; count: number }[]): number {
    return ingredients.reduce((sum, ingredient) => sum + ingredient.count, 0);
}

function normalizeIngredients(ingredients: { name: string; count: number }[]): { name: string; count: number }[] {
...
function simulateSuperLuckExpected(
    ...
    const ingredientCountPerTrigger = getSkillValue('Ingredient Draw S (Super Luck)', pokemon.iv.skillLevel);
    const dreamShardPerOne = getSkillSubValue('Ingredient Draw S (Super Luck)', pokemon.iv.skillLevel);
    ...
    for (let i = 0; i < triggerCount; i++) {
        ...
        if (roll < superLuckIngRate) {
            ...
            if (ingredient) {
                ingredientMap.set(ingredient, (ingredientMap.get(ingredient) ?? 0) + ingredientCountPerTrigger);
            }
            continue;
        }
    ...
    return {
        ingredients: Array.from(ingredientMap.entries()).map(([name, count]) => ({ name, count })),
        dreamShardCount,
    };
}
...
function simulateHyperCutterExpected(
    ...
    triggerCount: number,
    seed: number
): { ingredients: { name: string; count: number }[]; greatSuccessCount: number } {
    const random = new SeededRandom(seed);
    const ingredientPool = getIngredientDrawPool(pokemon);
    const ingredientCountPerTrigger = getSkillValue('Ingredient Draw S (Hyper Cutter)', pokemon.iv.skillLevel);
    const ingredientMap = new Map<string, number>();
    ...
    for (let i = 0; i < triggerCount; i++) {
        ...
        ingredientMap.set(ingredient, (ingredientMap.get(ingredient) ?? 0) + ingredientCountPerTrigger * multiplier);
    }
...
): { directEP: number; stockpileAfter: number; storeCount: number; spitCount: number } {
...
function calculateBerryStrength(type: PokemonType, level: number): number {
    ...
    return Math.max(baseStrength[type] + level - 1, Math.round(Math.pow(1.025, level - 1) * baseStrength[type]));
}
...
describe('SkillEffectProcessor', () => {
    ...
    it('Ingredient Magnet S (Plus)をingredientMagnetカテゴリに分類する', () => {
        expect(classifySkill('Ingredient Magnet S (Plus)')).toBe('ingredientMagnet');
    });

    it('Ingredient Magnet S (Present)をingredientMagnetカテゴリに分類する', () => {
        expect(classifySkill('Ingredient Magnet S (Present)')).toBe('ingredientMagnet');
    });
    ...
    it('Ingredient Draw S (Super Luck)をingredientDrawカテゴリに分類する', () => {
        expect(classifySkill('Ingredient Draw S (Super Luck)')).toBe('ingredientDraw');
    });

    it('Ingredient Draw S (Hyper Cutter)をingredientDrawカテゴリに分類する', () => {
        expect(classifySkill('Ingredient Draw S (Hyper Cutter)')).toBe('ingredientDraw');
    });
    ...
    it('Energy for Everyone S (Lunar Blessing)をteamEnergyカテゴリに分類する', () => {
        expect(classifySkill('Energy for Everyone S (Lunar Blessing)')).toBe('teamEnergy');
    });

    it('Energy for Everyone S (Berry Juice)をteamEnergyカテゴリに分類する', () => {
        expect(classifySkill('Energy for Everyone S (Berry Juice)')).toBe('teamEnergy');
    });
    ...
    it('Ingredient Magnet/Drawは最大倍率を適用する', () => {
        ...
        expect(magnetTotal).toBe(Math.floor(getSkillValue('Ingredient Magnet S', 2) * 1.5));
        expect(drawTotal).toBe(Math.floor(getSkillValue('Ingredient Draw S', 3) * 1.5));
    });
    ...
    it('Dream Shard倍率を反映する', () => {
        ...
        expect(result.dreamShardCount).toBe(getSkillValue('Dream Shard Magnet S', 4) * 3 * 2);
    });
    ...
    it('Berry Burst倍率を反映する', () => {
        ...
        const expectedBerryEP = calculateBerryStrength(caster.iv.pokemon.type, caster.iv.level) * boostedBerryCount;
    ...
    it('Cooking Power-Up S: 鍋容量増加量がスキル値×発動回数と一致する', () => {
        ...
        expect(result.cookingPotCapacityIncrease).toBe(getSkillValue('Cooking Power-Up S', 5) * triggerCount);
    ...
    it('Cooking Power-Up S (Minus): 条件不成立なら追加回復しない', () => {
        ...
        expect(result.cookingPotCapacityIncrease).toBe(getSkillValue('Cooking Power-Up S (Minus)', 4) * 5);
        expect(result.cookingMinusEvents).toHaveLength(0);
        expect(Array.from(result.cookingMinusTargets.values()).reduce((sum, v) => sum + v, 0)).toBe(0);
    });
    ...
    it('Cooking Power-Up S (Minus): 条件成立なら対象回復イベントを記録する', () => {
        ...
        expect(result1.cookingMinusEvents.every(event => event.recovery === expectedRecovery)).toBe(true);
        expect(result1.cookingMinusEvents.every(event => teamMembers.some(member => member.id === event.targetPokemonId))).toBe(true);
        expect(Array.from(result1.cookingMinusTargets.values()).reduce((sum, v) => sum + v, 0))
    ...
    it('Tasty Chance S: 上昇率がスキル値×発動回数と一致する', () => {
        ...
        expect(result.tastyChanceIncreasePercent).toBe(getSkillValue('Tasty Chance S', 6) * triggerCount);
    ...
    it('Dream Shard Magnet S (Random): 同一seedで再現可能かつ範囲内', () => {
        ...
        const [minShard, maxShard] = getSkillRandomRange('Dream Shard Magnet S (Random)', 6);
    ...
    it('Metronome: 発動者Lv7でmaxLv6スキルを引いた場合はLv6で処理する', () => {
        ...
        expect(result.teamEnergyRecoveryPerMember).toBe(getSkillValue('Energy for Everyone S', 6));
        expect(result.proxySkillEvents).toHaveLength(1);
        expect(result.proxySkillEvents[0]?.resolvedSkillName).toBe('Energy for Everyone S');
    ...
    it('Metronome: Charge Strength S (Stockpile)は強制ではきだすとして処理する', () => {
        ...
        expect(result.proxySkillEvents).toHaveLength(1);
        expect(result.proxySkillEvents[0]?.resolvedSkillName).toBe('Charge Strength S (Stockpile)');
    ...
    it('Skill Copy: コピー先がSkill Copy系列ならCharge Strength Sへフォールバックする', () => {
        ...
        expect(result.proxySkillEvents).toHaveLength(1);
        expect(result.proxySkillEvents[0]?.triggeredSkillName).toBe('Skill Copy (Mimic)');
        expect(result.proxySkillEvents[0]?.resolvedSkillName).toBe('Charge Strength S');
    ...
    it('Berry Burst: 発動者+他メンバー各体のきのみEPを合算する', () => {
        ...
        const expectedPerTrigger = teamMembers.reduce((sum, member) => {
            const berryCount = member.id === caster.id ? selfBerryCount : otherBerryCount;
            return sum + calculateBerryStrength(member.iv.pokemon.type, member.iv.level) * berryCount;
        }, 0);
    ...
    it('Ingredient Magnet S 1回発動で合計個数がスキル値と一致する', () => {
        ...
        const expectedTotal = getSkillValue('Ingredient Magnet S', pokemon.iv.skillLevel);
    ...
    it('Ingredient Magnet S 1回発動で3種類の食材を獲得しunknownを含まない', () => {
        ...
        expect(result.skillIngredients.every(ingredient => ingredient.count > 0)).toBe(true);
        expect(result.skillIngredients.some(ingredient => ingredient.name.startsWith('unknown'))).toBe(false);
    });
    ...
    it('Ingredient Magnet S (Plus) は同居条件なしでは追加食材を付与しない', () => {
        ...
        const expectedTotal = getSkillValue('Ingredient Magnet S (Plus)', pokemon.iv.skillLevel);
        expect(totalIngredientCount(withoutTeammate.skillIngredients)).toBe(expectedTotal);
    });
    ...
    it('Ingredient Magnet S (Plus) はPlus/Minus同居で第1食材を追加する', () => {
        ...
        const bonus = getSkillSubValue('Ingredient Magnet S (Plus)', pokemon.iv.skillLevel, firstIngredient);
    ...
    it('Ingredient Magnet S (Present) はアメを再現可能に計算する', () => {
        ...
        const result1 = processSkillTriggers(pokemon, 30, 50, new SeededRandom(55667), []);
        const result2 = processSkillTriggers(pokemon, 30, 50, new SeededRandom(55667), []);

        expect(normalizeIngredients(result1.skillIngredients)).toEqual(normalizeIngredients(result2.skillIngredients));
    ...
    it('Ingredient Magnet S (Present) は十分な試行数でアメを獲得する', () => {
        const pokemon = createIngredientMagnetPresentPokemon(6);
        const result = processSkillTriggers(pokemon, 100, 50, new SeededRandom(77889), []);
    ...
    it('同一seedならIngredient Magnet Sの結果が再現される', () => {
        ...
        const result1 = processSkillTriggers(pokemon, 2, 50, new SeededRandom(24680), []);
        const result2 = processSkillTriggers(pokemon, 2, 50, new SeededRandom(24680), []);
    ...
    it('異なるseedではIngredient Magnet Sの配分が変わるケースがある', () => {
        ...
        const baseline = normalizeIngredients(
            processSkillTriggers(pokemon, 2, 50, new SeededRandom(11111), []).skillIngredients
        );

        let foundDifferent = false;
        for (let seed = 11112; seed <= 11140; seed++) {
            const current = normalizeIngredients(
                processSkillTriggers(pokemon, 2, 50, new SeededRandom(seed), []).skillIngredients
            );
    ...
    it('Ingredient Draw S: 1回発動で自身の食材プールから1種をスキル値分獲得する', () => {
        ...
        const expectedCount = getSkillValue('Ingredient Draw S', pokemon.iv.skillLevel);
        const ingredientPool = getIngredientDrawPool(pokemon);

        expect(totalIngredientCount(result.skillIngredients)).toBe(expectedCount);
        expect(result.skillIngredients).toHaveLength(1);
        expect(result.skillIngredients.every(ingredient => ingredientPool.includes(ingredient.name))).toBe(true);
        expect(result.directEP).toBe(0);
    });
    ...
    it('Ingredient Draw S (Super Luck): 同一seedで食材/ゆめのかけら分岐が再現される', () => {
        ...
        expect(normalizeIngredients(result.skillIngredients)).toEqual(normalizeIngredients(expected.ingredients));
    ...
    it('Ingredient Draw S (Hyper Cutter): 同一seedで大成功回数と2倍獲得が再現される', () => {
        ...
        expect(normalizeIngredients(result.skillIngredients)).toEqual(normalizeIngredients(expected.ingredients));
        expect(result.ingredientDrawGreatSuccessCount).toBe(expected.greatSuccessCount);
    ...
    it('Bad Dreams: EPは固定値×発動回数で計算される', () => {
        ...
        expect(result.directEP).toBe(getSkillValue('Charge Strength M (Bad Dreams)', 3) * triggerCount);
    });
    ...
    it('Bad Dreams: 悪タイプ以外のみを対象としてヒット回数を計算する', () => {
        ...
        const nonDarkCount = teamMembers.filter(member => member.iv.pokemon.type !== 'dark').length;
    ...
    it('Energizing Cheer S: 回復総量がスキル値×発動回数と一致する', () => {
        ...
        const expectedRecovery = getSkillValue('Energizing Cheer S', caster.iv.skillLevel) * triggerCount;
        const actualRecovery = Array.from(result.energizingCheerTargets.values()).reduce((sum, v) => sum + v, 0);
    ...
    it('Nuzzle: 回復イベントが対象付きで記録される', () => {
        ...
        expect(result.energizingCheerEvents.every(event => teamMembers.some(member => member.id === event.targetPokemonId))).toBe(true);
    });
    ...
    it('Energy for Everyone S: 全員回復量がスキル値×発動回数と一致する', () => {
        ...
        expect(result.teamEnergyRecoveryPerMember).toBe(getSkillValue('Energy for Everyone S', 4) * triggerCount);
        expect(result.directEP).toBe(0);
    });
    ...
    it('Energy for Everyone S (Berry Juice): 同一seedで再現可能に確率判定される', () => {
        ...
        expect(result1.teamEnergyRecoveryPerMember).toBe(getSkillValue('Energy for Everyone S (Berry Juice)', 5) * triggerCount);
    ...
    it('Energy for Everyone S (Lunar Blessing): サイキック種族数に応じて追加EPを獲得する', () => {
        ...
        const { myBerryCount, othersBerryCount } = getLunarBlessingBerryCount(3, Math.min(Math.max(psychicSpeciesCount, 1), 5));
        const expectedPerTrigger = teamMembers.reduce((sum, member) => {
            const berryCount = member.id === caster.id ? myBerryCount : othersBerryCount;
            return sum + calculateBerryStrength(member.iv.pokemon.type, member.iv.level) * berryCount;
        }, 0);

        expect(result.teamEnergyRecoveryPerMember).toBe(getSkillValue('Energy for Everyone S (Lunar Blessing)', 3) * triggerCount);
        expect(result.directEP).toBe(expectedPerTrigger * triggerCount);
    });
    ...
    it('Extra Helpful S: 発動ごとに対象1体へ追加おてつだいを適用する', () => {
        ...
        expect(result.supportHelpEvents.every(event => event.source === 'extraHelpful')).toBe(true);
        expect(result.supportHelpEvents.every(event => event.helpCount === getSkillValue('Extra Helpful S', 4))).toBe(true);
    ...
    it('Helper Boost: チーム全員へ同回数のおてつだいを適用する', () => {
        ...
        const expectedHelpCount = getSkillValue('Helper Boost', 6, Math.min(Math.max(sameTypeSpeciesCount, 1), 5));
        ...
        expect(result.supportHelpEvents.length).toBe(triggerCount * teamMembers.length);
        expect(result.supportHelpEvents.every(event => event.source === 'helperBoost')).toBe(true);
