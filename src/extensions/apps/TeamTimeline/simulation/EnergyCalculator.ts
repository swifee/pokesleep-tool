...
export function calculateEnergy(input: EnergyInput): EnergyOutput {
	const { startEnergy, durationMinutes, isMealSlot, teamSkillRecovery } = input;
	...
	const averageEnergy =
		(calculateWeightedEfficiency(energyAfterRecovery, durationMinutes) *
			energyAfterRecovery) /
		getEfficiencyMultiplier(energyAfterRecovery);
 ...
 * @returns 起床回復計算の出力
 */
export function calculateWakeRecovery(
	...
	// 上限を決定
	const maxEnergy = hasEnergyRecoveryBonus
		? ERB_MAX_ENERGY
		: DEFAULT_MAX_ENERGY;
