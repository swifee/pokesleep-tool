...
import { IngredientResult, TimeSlotResult, DailySummary } from '../types/TimeSlotTypes';
...
describe('EnergyPointCalculator', () => {
    ...
    describe('calculateDailySummary', () => {
        it('一日合計を正しく計算', () => {
            ...
            expect(summary.totalEP).toBe(summary.berryEP + summary.ingredientEP + summary.skillEP);
        ...
        it('Extra Helpful SはdirectSkillEPをskillEPとして集計する', () => {
            const supportSkillPokemonData = pokemons.find(p => p.skill === 'Extra Helpful S');
            expect(supportSkillPokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: supportSkillPokemonData!.name, skillLevel: 4 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Cooking Power-Up Sは発動回数があってもskillEPを0として集計する', () => {
            const nonEPSkillPokemonData = pokemons.find(p => p.skill === 'Cooking Power-Up S');
            expect(nonEPSkillPokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: nonEPSkillPokemonData!.name, skillLevel: 5 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Dream Shard Magnet Sは発動回数があってもskillEPを0として集計し、ゆめのかけらを合計する', () => {
            const dreamShardPokemonData = pokemons.find(p => p.skill === 'Dream Shard Magnet S');
            expect(dreamShardPokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: dreamShardPokemonData!.name, skillLevel: 6 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Ingredient Draw SはskillEPを0として食材EPのみ加算する', () => {
            const skillPokemonData = pokemons.find(p => p.skill === 'Ingredient Draw S');
        ...
        it('Ingredient Draw S (Super Luck)のゆめのかけらはskillEPへ加算しない', () => {
            const skillPokemonData = pokemons.find(p => p.skill === 'Ingredient Draw S (Super Luck)');
            expect(skillPokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: skillPokemonData!.name, skillLevel: 7 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Ingredient Draw S (Hyper Cutter)は大成功があってもskillEPへ加算しない', () => {
            const skillPokemonData = pokemons.find(p => p.skill === 'Ingredient Draw S (Hyper Cutter)');
            expect(skillPokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: skillPokemonData!.name, skillLevel: 7 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Berry BurstはdirectSkillEPをskillEPとして集計する', () => {
            ...
            const iv = new PokemonIv({ pokemonName: berryBurstPokemonData!.name, skillLevel: 4 });
            const pokemon = new PokemonBoxItem(iv);
        ...
        it('Metronomeはproxy発動由来のdirectSkillEPをskillEPとして集計する', () => {
            const metronomePokemonData = pokemons.find(p => p.skill === 'Metronome');
            expect(metronomePokemonData).toBeDefined();
            const iv = new PokemonIv({ pokemonName: metronomePokemonData!.name, skillLevel: 7 });
            const pokemon = new PokemonBoxItem(iv);
    ...
    describe('calculateTeamSummary', () => {
        it('チーム合計を正しく計算', () => {
            ...
            expect(teamSummary.totalIngredients.find(i => i.name === 'apple')?.count).toBe(18);
            expect(teamSummary.totalIngredients.find(i => i.name === 'milk')?.count).toBe(5);
            expect(teamSummary.totalIngredients.find(i => i.name === 'honey')?.count).toBe(6);
        });
