...
import {
	calculateHelp,
	getEffectiveMaxInventory,
	type HelpBonusContext,
	type HelpInput,
} from "./HelpCalculator";
...
function addIngredientResultsToMap(
	...
	for (const ingredient of ingredients) {
		target.set(
			ingredient.name,
			(target.get(ingredient.name) ?? 0) + ingredient.count,
		);
	}
}

function ingredientMapToResultArray(
	map: ReadonlyMap<IngredientName, number>,
): { name: IngredientName; count: number }[] {
	return [...map.entries()].map(([name, count]) => ({ name, count }));
}

function sumIngredientCounts(
	ingredients: readonly { count: number }[],
): number {
...
export function getTeamEnergyRecoveryBonusCount(
	...
	return team.filter(
		(pokemon) =>
			...
			pokemon.iv.activeSubSkills.some(
				(s) => s.name === "Energy Recovery Bonus",
			),
	).length;
}
...
function hasEnergyRecoveryBonus(
	...
	return pokemon.iv.activeSubSkills.some(
		(s) => s.name === "Energy Recovery Bonus",
	);
}
...
function getBaselineUsageCounts(
...
function applyExtraIngredientsToBaselineEvents(
	...
	const bonusMultiplier =
		(1 + bonusSettings.fieldBonus / 100) * (1 + eventBonus / 100);
	...
	for (const expandedSlot of expandedSlots) {
		...
		const isMealSlot = slot.hasMeal !== undefined ? slot.hasMeal : false;
		if (!isMealSlot) {
			continue;
		}
		...
		let remainingPotCapacity = normalizeDiscreteCount(
			baselineEvent.remainingPotCapacity,
		);
		...
		const replayedExtraIngredientsUsed = deductIngredientCountsFromBag(
			bag,
			appliedExtraCounts,
		);
		...
		if (baselineEvent.recipeName == null) {
			if (extraRawStrength > BAG_COUNT_EPSILON) {
				const mixedRecipeName =
					MIXED_RECIPE_NAME_BY_CATEGORY[cookingSettings.category];
		...
		updatedEvents.push({
			...
			bagIngredientsBeforeCookingWithoutExtra:
				baselineEvent.bagIngredientsBeforeCooking,
...
function runCookingPostProcess(
	...
	const totalInitialIngredientEP =
		computeInitialIngredientAttributedEP(allEvents);
...
export function runSimulation(input: SimulationInput): SimulationResult {
	...
	const keepDisabledPokemonTargetable =
		analysisOptions?.keepDisabledPokemonTargetable !== false;
	...
	const normalizedTeam = team.map((pokemon) =>
		pokemon ? getSimulationPokemon(pokemon) : null,
	);

	// 1. 有効なポケモンのみ抽出
	const validTeam: PokemonBoxItem[] = normalizedTeam.filter(
		(p): p is PokemonBoxItem => p !== null,
	);
	const activeTeamAtStart = validTeam.filter(
	...
	const getPokemonBonusContext = (
		...
		const built = buildPokemonBonusContext(
			pokemon,
			bonusSettings,
			strengthParameter,
		);
	...
	swaps.forEach((swap) => {
		const dayIndex = typeof swap.dayIndex === "number" ? swap.dayIndex : 0;
		swapCellKeySet.add(
			toTimelineCellKey(dayIndex, swap.slotId, swap.teamSlotIndex),
		);
	});
	const activeNoCollectCellKeySet = new Set<string>();
	noCollectCells.forEach((cell) => {
		const key = toTimelineCellKey(
			cell.dayIndex,
			cell.slotId,
			cell.teamSlotIndex,
		);
	...
	for (let i = 0; i < expandedSlots.length; i++) {
		...
		if (getDisplayLabel(slot) === "wake") {
			...
			pokemonStates.forEach((state) => {
				...
				const passiveWakeInput: WakeRecoveryInput = {
					...
					hasEnergyRecoveryBonus: hasEnergyRecoveryBonus(
						state.pokemon,
				...
				if (state.currentEnergy < ENERGY_MAX_NORMAL) {
					const passiveRecovery =
						passiveWakeOutput.recoveredEnergy / INACTIVE_WAKE_RECOVERY_DIVISOR;
					state.currentEnergy = Math.min(
						state.currentEnergy + passiveRecovery,
						ENERGY_MAX_NORMAL,
					);
		...
		const helpOutputs: {
			state: PokemonState;
			helpOutput: ReturnType<typeof calculateHelp>;
		}[] = [];
		...
		const wakeAndMealRecoveryMap = new Map<
			number,
			{
		...
		for (const { state } of helpOutputs) {
			...
			if (getDisplayLabel(slot) === "wake" && state.sleepStartTime) {
				const sleepMinutes = calculateDuration(state.sleepStartTime, slot.time);
				selfHasErb = hasEnergyRecoveryBonus(
					state.pokemon,
					energyRecoveryOptions,
				);
				const teamErbCount = getTeamEnergyRecoveryBonusCount(
					currentActiveTeam,
					energyRecoveryOptions,
				);
		...
		for (let idx = 0; idx < helpOutputs.length; idx++) {
			...
			if (wakeAndMealData === undefined) {
				throw new Error("Missing wake and meal recovery data");
			}
			...
			const skillResult = processSkillTriggers(
				...
				teamSkillBonusContext,
				{
					...
					disabledPokemonIds,
					suppressEnergyDelta: suppressEnergyDeltaSkillPokemonIds.has(
						state.pokemon.id,
					),
				},
			);
		...
		for (const { state, helpOutput } of helpOutputs) {
			...
			const wakeAndMealData = wakeAndMealRecoveryMap.get(state.pokemon.id);
			if (skillResult === undefined || wakeAndMealData === undefined) {
				throw new Error("Missing per-pokemon simulation data");
			}
			const moonlightReceived = moonlightReceivedMap.get(state.pokemon.id) || 0;
			const energizingCheerReceived =
				energizingCheerReceivedMap.get(state.pokemon.id) || 0;
			const additionalRecoveryReceived =
				additionalRecoveryReceivedMap.get(state.pokemon.id) || 0;
			const cookingMinusReceived =
				cookingMinusReceivedMap.get(state.pokemon.id) || 0;
			...
			addIngredientResultsToMap(
				...
				skillIngredientsFromSkill,
			);
			state.inventoryCount =
				helpOutput.newInventory +
				sumIngredientCounts(skillIngredientsFromSkill);
			...
			if (isNoCollectEnabled) {
				const overflowInventoryCount = Math.max(
					0,
					state.inventoryCount - effectiveMaxInventory,
				);
				collectedBerryCount = Math.min(
					overflowInventoryCount,
					state.carriedBerryCount,
				);
				...
				collectedSkillIngredients = ingredientMapToResultArray(
					state.carriedSkillIngredients,
				);
			...
			const result: TimeSlotResult = {
				...
				moonlightGivenRecovery: Array.from(
					skillResult.moonlightTargets.values(),
				).reduce((sum, v) => sum + v, 0),
				moonlightEvents: Array.from(skillResult.moonlightTargets.entries()).map(
					([targetPokemonId, recovery]) => ({
						targetPokemonId,
						targetPokemonName:
							pokemonNameMap.get(targetPokemonId) ?? String(targetPokemonId),
						targetPokemonIdForm: pokemonIdFormMap.get(targetPokemonId),
						recovery,
					}),
				),
				moonlightReceivedRecovery: moonlightReceived,
				energizingCheerGivenRecovery: Array.from(
					skillResult.energizingCheerTargets.values(),
				).reduce((sum, v) => sum + v, 0),
				energizingCheerReceivedRecovery: energizingCheerReceived,
				energizingCheerEvents: skillResult.energizingCheerEvents.map(
					(event) => ({
						targetPokemonId: event.targetPokemonId,
						targetPokemonName:
							pokemonNameMap.get(event.targetPokemonId) ??
							String(event.targetPokemonId),
						targetPokemonIdForm: pokemonIdFormMap.get(event.targetPokemonId),
						recovery: event.recovery,
						source: event.source,
					}),
				),
				nuzzleTriggeredSkillEvents: skillResult.nuzzleTriggeredSkillEvents.map(
					(event) => ({
						targetPokemonId: event.targetPokemonId,
						targetPokemonName:
							pokemonNameMap.get(event.targetPokemonId) ??
				...
				proxySkillEvents: skillResult.proxySkillEvents.map((event) => ({
					...
					copiedFromPokemonName:
						event.copiedFromPokemonId !== undefined
							? (pokemonNameMap.get(event.copiedFromPokemonId) ??
								String(event.copiedFromPokemonId))
							: undefined,
				...
				ingredientDrawGreatSuccessCount:
					skillResult.ingredientDrawGreatSuccessCount,
				...
				teamEnergyRecoveryGivenPerMember:
					skillResult.teamEnergyRecoveryPerMember,
				teamEnergyRecoveryGivenTargetCount:
					skillResult.teamEnergyRecoveryPerMember > 0
		...
		for (const swap of slotSwaps) {
			...
			// 新しいポケモンを取得
			const newPokemon = box?.items.find(
				(item: PokemonBoxItem) => item.id === swap.newPokemonId,
			);
			...
			if (pokemonLastEnergy.has(simulationPokemon.id)) {
				// 2回目以降: 前回の最終げんきを引き継ぐ
				const previousEnergy = pokemonLastEnergy.get(simulationPokemon.id);
				if (previousEnergy === undefined) {
					throw new Error("Missing persisted energy for swapped pokemon");
				}
				startEnergy = previousEnergy;
			} else {
	...
	if (cookingResult) {
		for (const attribution of cookingResult.pokemonAttributions) {
			const summary = dailySummaries.find(
				(s) => s.pokemonId === attribution.pokemonId,
			);
			if (summary) {
