...
const INGREDIENT_MAGNET_CANDIDATES: readonly IngredientName[] =
	IngredientNames.filter((ingredient) => !ingredient.startsWith("unknown"));
...
/**
 * ポケモンのきのみ強度補正を求める。
 * 「きのみゾーン」展開中はマゴのみ（エスパータイプ）の補正を上乗せする。
 */
function resolveBerryStrengthBonus(
	member: PokemonBoxItem,
	bonusContext?: TeamSkillBonusContext,
): number {
	const baseBonus =
		bonusContext?.byPokemonId.get(member.id)?.berryStrengthBonus ?? 1;
	return (
		baseBonus *
		getBerryZoneMultiplierForType(
...
function calculateBerryStrength(type: PokemonType, level: number): number {
	...
	return Math.max(
		...
		Math.round(1.025 ** (level - 1) * baseStrength),
	);
}
...
function addIngredientCount(
	...
	ingredientMap.set(
		ingredientName,
		(ingredientMap.get(ingredientName) ?? 0) + count,
	);
}

function addNumberToMap(
...
function calculateDistributedBerryEp(
	...
	for (let i = 0; i < triggerCount; i++) {
		for (const member of activeTeam) {
			const berryCount =
				member.id === selfPokemonId ? selfBerryCount : otherBerryCount;
			if (berryCount <= 0) continue;
			const berryStrengthBonus = resolveBerryStrengthBonus(
				member,
				bonusContext,
			);
...
export function resolveSkillLevelForSkill(
	skill: MainSkillName,
...
function simulateSupportHelps(
	...
	const berryStrengthBonus = resolveBerryStrengthBonus(target, bonusContext);
	const fieldBonus = bonusContext?.fieldBonus ?? 0;
...
export function processSkillTriggers(
	...
	const skillName =
		proxySkillNameOverride ?? getEffectiveMainSkillName(pokemon);
	const pokemonBonus = bonusContext?.byPokemonId.get(pokemon.id);
	const skillLevel = resolveSkillLevelForSkill(
		skillName,
		proxySkillLevelOverride ??
			pokemon.iv.skillLevel + (pokemonBonus?.skillLevelBonus ?? 0),
	);
	const category = classifySkill(skillName);
	const activeNuzzleChainState = nuzzleChainState ?? {
		remaining: NUZZLE_MAX_CHAIN_TRIGGERS,
	};
	const defaultTeamMembers =
		teamMembers.length > 0 ? teamMembers : [pokemon, ...teammates];
	const targetableTeamMembers =
		analysisContext?.targetableTeamMembers !== undefined
	...
	const activeTeamMembers = activeTeamMemberIds
		? targetableTeamMembers.filter((member) =>
				activeTeamMemberIds.has(member.id),
			)
		: targetableTeamMembers;
	const targetableTeammates = targetableTeamMembers.filter(
		(member) => member.id !== pokemon.id,
	);
	const activeTeammates = activeTeamMembers.filter(
		(member) => member.id !== pokemon.id,
	);
	...
	let stockpileCount = Math.min(
		Math.max(currentStockpileCount, 0),
		STOCKPILE_MAX_COUNT,
	);
	...
	if (category === "selfEnergy") {
			...
			if (
				...
				if (random.chance(MOONLIGHT_TEAMMATE_PROBABILITY)) {
					...
					const clampedLevel = Math.min(
						...
						MOONLIGHT_TEAMMATE_VALUES.length,
					);
					const teammateRecovery =
						MOONLIGHT_TEAMMATE_VALUES[clampedLevel - 1] ?? 0;
		...
		if (skillName === "Charge Strength S (Stockpile)") {
			const levelIndex =
				Math.min(Math.max(skillLevel, 1), STOCKPILE_EP_TABLE.length) - 1;
			...
			const nonDarkCount = targetableTeamMembers.filter(
			...
			if (suppressEnergyDelta) {
				...
				badDreamsDamagePerTarget =
					BAD_DREAMS_DAMAGE_PER_HIT * skillTriggerCount;
		...
		const recoveryPerTrigger = getSkillValue(skillName, skillLevel);
		totalTeamRecovery = suppressEnergyDelta
			? 0
			: recoveryPerTrigger * skillTriggerCount;
		...
		if (
			...
			if (psychicSpeciesCount > 0) {
				const clampedSpeciesCount = Math.min(
					Math.max(psychicSpeciesCount, 1),
					5,
				);
		...
		if (activeTeam.length > 0) {
			for (let i = 0; i < skillTriggerCount; i++) {
				...
				energizingCheerEvents.push({
					...
					recovery: suppressEnergyDelta ? 0 : recoveryPerTrigger,
					source:
						skillName === "Energizing Cheer S (Nuzzle)" ? "nuzzle" : "cheer",
				});
				...
				if (
					...
					const targetSkillTriggerBonus =
						bonusContext?.byPokemonId.get(target.id)?.skillTriggerBonus ?? 1;
					const targetSkillRate = Math.min(
						1,
						target.iv.skillRate * targetSkillTriggerBonus,
					);
					const triggerProbability = 1 - (1 - targetSkillRate) ** skillLevel;
					if (random.chance(triggerProbability)) {
						...
						totalCookingPotCapacityIncrease +=
							nestedResult.cookingPotCapacityIncrease;
						totalTastyChanceIncreasePercent +=
							nestedResult.tastyChanceIncreasePercent;
						totalDreamShardCount += nestedResult.dreamShardCount;
						totalIngredientDrawGreatSuccessCount +=
							nestedResult.ingredientDrawGreatSuccessCount;
						totalBerryBurstGreatSuccessCount +=
							nestedResult.berryBurstGreatSuccessCount;
						berryBurstDisguiseLockedState =
							nestedResult.berryBurstDisguiseLockedAfter;
						...
						for (const [
							targetId,
							recovery,
						] of nestedResult.moonlightTargets.entries()) {
							addNumberToMap(moonlightTargets, targetId, recovery);
						}
						for (const [
							targetId,
							recovery,
						] of nestedResult.energizingCheerTargets.entries()) {
							addNumberToMap(energizingCheerTargets, targetId, recovery);
						}
						for (const [
							targetId,
							recovery,
						] of nestedResult.additionalRecoveryTargets.entries()) {
							addNumberToMap(additionalRecoveryTargets, targetId, recovery);
						}
						for (const [
							targetId,
							recovery,
						] of nestedResult.cookingMinusTargets.entries()) {
						...
						nuzzleTriggeredSkillEvents.push(
							...nestedResult.nuzzleTriggeredSkillEvents,
						);
						...
						addNumberToMap(
							additionalRecoveryTargets,
							target.id,
							nestedResult.selfEnergyRecovery,
						);
					}
				}
			}
		...
		if (targetableTeam.length > 0) {
			for (let i = 0; i < skillTriggerCount; i++) {
				...
				const sameTypeSpeciesCount = new Set(
					activeTeamMembers
						.filter(
							(member) => member.iv.pokemon.type === pokemon.iv.pokemon.type,
						)
						.map((member) => member.iv.pokemonName),
				).size;
				const clampedSpeciesCount = Math.min(
					Math.max(sameTypeSpeciesCount, 1),
					5,
				);
				const helpCount = getSkillValue(
					"Helper Boost",
					skillLevel,
					clampedSpeciesCount,
				);
		...
		const totalCountPerTrigger = Math.floor(
			getSkillValue(skillName, skillLevel) * ingredientMagnetMultiplier,
		);
		const baseCount = Math.floor(
			totalCountPerTrigger / INGREDIENT_MAGNET_PICK_COUNT,
		);
		const remainderCount = totalCountPerTrigger % INGREDIENT_MAGNET_PICK_COUNT;
		const shouldApplyPlusBonus =
			skillName === "Ingredient Magnet S (Plus)" &&
			activeTeammates.some((teammate) =>
				PLUS_TRIGGER_SKILLS.has(getEffectiveMainSkillName(teammate)),
			);
		...
		const dreamShardMultiplier =
			ingredientDrawMultiplier *
			(pokemonBonus?.mainSkillDreamShardMultiplier ?? 1);
		const ingredientPerTrigger = Math.floor(
			getSkillValue(skillName, skillLevel) * ingredientDrawMultiplier,
		);
		const ingredientDrawPool = getIngredientDrawPool(pokemon);
		const dreamShardPerOne =
			skillName === "Ingredient Draw S (Super Luck)"
		...
		for (let i = 0; i < skillTriggerCount; i++) {
			...
			if (skillName === "Ingredient Draw S (Super Luck)") {
				...
				skillName === "Ingredient Draw S (Hyper Cutter)" &&
				random.chance(hyperCutterSuccess)
			) {
			...
			const selectedIngredient =
				ingredientDrawPool[ingredientIndex] ?? ingredientDrawPool[0];
			...
			addIngredientCount(
				skillIngredientMap,
				selectedIngredient,
				ingredientPerTrigger * ingredientMultiplier,
			);
		}
		...
		if (skillName === "Tasty Chance S") {
			...
			const tastyChanceIncreasePerTrigger = getSkillSubValue(
				skillName,
				skillLevel,
			);
			totalTastyChanceIncreasePercent =
				tastyChanceIncreasePerTrigger * skillTriggerCount;
			...
			for (let i = 0; i < skillTriggerCount; i++) {
				const selectedIngredients = random
					...
					.slice(0, INGREDIENT_MAGNET_PICK_COUNT);
				const baseCount = Math.floor(
					ingredientCountPerTrigger / INGREDIENT_MAGNET_PICK_COUNT,
				);
			...
			totalCookingPotCapacityIncrease =
				potIncreasePerTrigger * skillTriggerCount;
			...
			if (skillName === "Cooking Power-Up S (Minus)") {
				...
				const recoveryPerTrigger = getSkillSubValue(
					"Cooking Power-Up S (Minus)",
					skillLevel,
				);
				...
				if (
					hasOtherPlusMinus &&
					recoveryPerTrigger > 0 &&
					targetableTeam.length > 0
				) {
					for (let i = 0; i < skillTriggerCount; i++) {
						const targetIndex = random.nextInt(0, targetableTeam.length - 1);
						const target = targetableTeam[targetIndex];
		...
		const dreamShardMultiplier =
			(pokemonBonus?.dreamShardMultiplier ?? 1) *
			(pokemonBonus?.mainSkillDreamShardMultiplier ?? 1);
		...
		for (let i = 0; i < skillTriggerCount; i++) {
			...
			if (skillName === "Metronome") {
				const metronomeIndex = random.nextInt(
					0,
					METRONOME_SKILL_POOL.length - 1,
				);
				...
				const copiedTarget =
					copyCandidates[random.nextInt(0, copyCandidates.length - 1)];
				if (!copiedTarget) {
					continue;
				}
			...
			const nestedResult = processSkillTriggers(
				...
				resolvedSkillLevel,
				source === "metronome" &&
					resolvedSkillName === "Charge Strength S (Stockpile)",
				bonusContext,
				analysisContext,
			);
	...
	const skillIngredients: IngredientResult[] = Array.from(
		skillIngredientMap.entries(),
	).map(([name, count]) => ({
		name,
		count,
	}));
