...
function buildCookingDayKey(
	...
	const eventSignature = daySummary.events
		.map((event) => `${event.mealSlotId}-${event.recipeName ?? "skip"}`)
		.join("|");
	return `${eventSignature}-${daySummary.totalCookingEP}-${daySummary.greatSuccessCount}`;
}

function buildCookingEventKey(
	event: NonNullable<
		CookingSimulationResult["dailySummaries"][number]
	>["events"][number],
): string {
	return [
		event.mealSlotId,
		event.recipeName ?? "skip",
		event.cookingEP,
		event.remainingPotCapacity,
		event.isGreatSuccess ? "great" : "normal",
	].join("-");
}
...
function computePostExtraLeftoverTotals(
	...
	for (const [name, fixedCount] of Object.entries(fixedTotals)) {
		...
		result[ingredientName] = Math.max(
			0,
			baseCount - (extraUsageTotals[ingredientName] ?? 0),
		);
	}
	return result;
}
...
const TeamSummaryRow = React.memo(
	({
		...
		const groupedAverageIngredients = groupLowDailyIngredientsForAverage(
			sortedIngredients,
			convertedDayDivisor,
		);
		...
		const totalDreamShardCount = convertByMode(
			teamSummary.totalDreamShardCount,
		);
		...
		const canShowToggle =
			layoutMode === "details" && showValueModeToggle && onValueModeChange;
		const sortedAverageCookingRecipes = averageCookingSummary
			...
			: [];
		const groupedAverageCookingRecipes = groupAverageCookingRecipes(
			sortedAverageCookingRecipes,
		);
		...
		const detailsLeftoverTotals = cookingResult
			? includeExtraUsage
				...
				: computePostExtraLeftoverTotals(
						cookingResult.leftoverIngredients.total,
						cookingResult.events,
					)
			: {};
		const hasDetailsLeftoverIngredients = Object.values(
			detailsLeftoverTotals,
		).some((count) => (count ?? 0) > 0);
		const averageLeftoverIngredients = averageCookingSummary
			? includeExtraUsage
				? averageCookingSummary.leftoverIngredients
				: (averageCookingSummary.leftoverIngredientsAfterExtra ??
					averageCookingSummary.leftoverIngredients)
			: [];
		const shouldShowLeftoverToggle =
			showLeftoverIncludeExtraUsageToggle &&
			(layoutMode === "details"
				? Boolean(cookingResult)
				: Boolean(averageCookingSummary));
				...
				<ContentCell>
					<EPPanel>
						<EPLine>
							<EPItem data-testid="team-summary-ep-item-berry">
								<TeamTimelineIcon
									name="berry"
									data-testid="team-summary-ep-icon-berry"
								/>
							...
							<EPItem data-testid="team-summary-ep-item-skill">
								<TeamTimelineIcon
									name="skill"
									data-testid="team-summary-ep-icon-skill"
								/>
								<EpValue value={formatSummaryEp(totalSkillEP)} />
							</EPItem>
							<EPItem
								data-testid={`team-summary-ep-item-${teamSummary.totalCookingEP != null ? "cooking" : "ingredient"}`}
							>
								<TeamTimelineIcon
									name="cooking"
									data-testid="team-summary-ep-icon-cooking"
								/>
					...
					<IngredientSection>
						...
						<MetaItem>
							1食平均 :{" "}
							<span className="value">
								{formatIngredientCount(ingredientPerMeal)}
							</span>
						</MetaItem>
						...
						{totalCookingPotCapacityIncrease > 0 && (
							<MetaItem>
								鍋容量 : +{formatSummaryNumber(totalCookingPotCapacityIncrease)}
							</MetaItem>
						)}
						{totalDreamShardCount > 0 && (
							<MetaItem>
								<TeamTimelineIcon
									name="dream"
									data-testid="team-summary-meta-icon-dream"
								/>
					...
					{layoutMode === "details" && cookingResult && (
						<CookingSection>
							...
							{cookingResult.dailySummaries.map((daySummary, dayIndex) => (
								<DayCookingGroup key={buildCookingDayKey(daySummary)}>
									{cookingResult.dailySummaries.length > 1 && (
										<DayCookingLabel>{dayIndex + 1}日目</DayCookingLabel>
									)}
									{daySummary.events.map((event) => {
										const remainingPotCapacity = Math.max(
											0,
											Math.round(event.remainingPotCapacity),
										);
										return (
											<CookingEventLine key={buildCookingEventKey(event)}>
												{event.isGreatSuccess && (
													<GreatSuccessMark>&#x2757;</GreatSuccessMark>
												)}
												<span className="recipe-name">
													{event.recipeName == null
														? "スキップ"
														: t(
																`TeamTimeline.recipe ${event.recipeName}`,
																event.recipeName,
															)}
												</span>
							...
							{shouldShowLeftoverToggle && (
								<LeftoverToggleLabel>
									<LeftoverToggleInput
										...
										aria-label={t(
											"TeamTimeline.cooking include extra usage in leftover",
											"追加食材使用分を含む",
										)}
							...
							<InitialIngredientEpLine>
								{t(
									"TeamTimeline.cooking initial ingredient ep total",
									"初期食材由来EP合計",
								)}
								{" : "}
								<EpValue
									value={formatSummaryEp(
										convertByMode(cookingResult.totalInitialIngredientEP ?? 0),
									)}
								/>
							</InitialIngredientEpLine>
						</CookingSection>
							...
							<CookingSection>
								...
								{groupedAverageCookingRecipes.visibleRecipes.map((recipe) => (
									<AverageCookingRecipeLine key={recipe.recipeName}>
										<span className="recipe-text">
											{t(
												`TeamTimeline.recipe ${recipe.recipeName}`,
												recipe.recipeName,
											)}
											{" : "}
											平均
											<EpValue
												value={formatSummaryEp(recipe.averageCookingEP)}
											/>{" "}
											× {formatAverageRecipeCount(recipe.averageCount)}回
										</span>
								...
								<InitialIngredientEpLine>
									...
									<EpValue
										value={formatSummaryEp(
											convertByMode(
												averageCookingSummary.averageInitialIngredientEP ?? 0,
											),
										)}
									/>
								</InitialIngredientEpLine>
