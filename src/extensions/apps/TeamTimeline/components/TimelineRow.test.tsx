	...
	default: ({
		...
		<div data-testid="timeline-cell">
			...
			<span data-testid="pokemon-id-form">
				{pokemonIdForm === undefined ? "" : String(pokemonIdForm)}
			</span>
...
describe("TimelineRow label rendering", () => {
	...
	it("does not render a right border on the time cell", () => {
		...
		expect((timeCell as HTMLElement).getAttribute("data-right-divider")).toBe(
			"off",
		);
	});
});
...
describe("TimelineRow swap rendering", () => {
	...
	it("keeps current pokemon icon on the swap cell before simulation results", () => {
		...
		const firstCell = screen.getAllByTestId("timeline-cell")[0];
		expect(within(firstCell).getByTestId("pokemon-id-form").textContent).toBe(
			"260",
		);
	});
	...
	it("passes no-collect enabled flag to matched non-swap cell", () => {
		...
		const firstCell = screen.getAllByTestId("timeline-cell")[0];
		expect(within(firstCell).getByTestId("no-collect-flag").textContent).toBe(
			"1",
		);
	});
