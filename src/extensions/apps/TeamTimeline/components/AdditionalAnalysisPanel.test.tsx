...
vi.mock("@mui/material", async () => {
	...
	return {
		...
		AccordionDetails: ({
			children,
			expanded,
		}: ChildrenProps & { expanded?: boolean }) =>
...
function renderPanel(
	overrides?: Partial<React.ComponentProps<typeof AdditionalAnalysisPanel>>,
) {
...
describe("AdditionalAnalysisPanel", () => {
	...
	it("shows sections after expanding accordion", () => {
		...
		expect(
			(screen.getByLabelText("quick-mode-switch") as HTMLInputElement).checked,
		).toBe(true);
	...
	it("shows help tooltip on hover for each section title", () => {
		...
		fireEvent.mouseEnter(
			screen.getByRole("button", { name: "貢献EPの説明を表示" }),
		);
	...
	it("uses fixed width for energy skill run-all button", () => {
		...
		expect(runAllButtons[1].getAttribute("data-sx")).toContain(
			'"width":"110px"',
		);
	});
	...
	it("renders energy skill rows as text", () => {
		...
		expect(
			screen.queryByRole("button", { name: member1.iv.pokemonName }),
		).toBeNull();
	...
	it("renders contribution rows as text", () => {
		...
		expect(
			screen.queryByRole("button", { name: "pokemons.Rattata" }),
		).toBeNull();
	});
	...
	it("shows empty target message and disables run-all button when no energy skill target exists", () => {
		...
		expect(
			screen.getByText("げんき変動スキルを持つメンバーがいません"),
		).toBeDefined();
	});
	...
	it("disables energy recovery bonus button when no target exists", () => {
		...
		expect(
			screen.getByText("げんき回復ボーナスを持つメンバーがいません"),
		).toBeDefined();
	});
	...
	it("disables helping bonus button when no target exists", () => {
		...
		expect(
			screen.getByText("おてつだいボーナスを持つメンバーがいません"),
		).toBeDefined();
	});
