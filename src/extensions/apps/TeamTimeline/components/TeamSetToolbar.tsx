...
function resolveTeamSlotKey(memberIndex: number): string {
	return TEAM_SLOT_KEYS[memberIndex] ?? `slot-${memberIndex + 1}`;
}
...
const TeamSetToolbar = React.memo(
	({
		...
		const safeActiveIndex = Math.max(
			0,
			Math.min(activeTeamSetIndex, Math.max(teamSets.length - 1, 0)),
		);
			...
			() =>
				new Map(
					teamSets.map((teamSet) => [
						teamSet.id,
						countUniqueSwapPokemonIds(teamSet),
					]),
				),
			[teamSets],
		);
		...
		return (
			<>
				...
				>
					...
					>
						{teamSets.map((teamSet, index) => {
							...
							const isStale =
								snapshot !== null &&
								snapshot.settingsHash !== currentSimulationContextHash;
							return (
								<MenuItem
									key={teamSet.id}
									value={index}
									data-testid={`team-set-menu-item-${index}`}
								>
									<Box
										sx={{
											display: "flex",
									...
									>
										...
										>
											...
											>
												{teamSet.team.map((member, memberIndex) =>
													member ? (
														<Box
															key={`${teamSet.id}-${resolveTeamSlotKey(memberIndex)}`}
												...
												{swapCount > 0 ? (
													...
													>
														{t(
															"TeamTimeline.team set swap count suffix",
															"+{{count}}",
															{ count: swapCount },
														)}
													</Typography>
										...
										>
											...
											>
												{snapshot ? (
													<EpValue
														value={formatSummaryEp(snapshot.averageTotalEP)}
													/>
												) : (
													""
												)}
											...
											<IconButton
												size="small"
												aria-label={t(
													"TeamTimeline.team set duplicate",
													"複製",
												)}
						...
						<MenuItem
							value={CREATE_MENU_VALUE}
							data-testid="team-set-create-menu-item"
						>
				...
				>
					<DialogTitle>
						{t("TeamTimeline.team set save settings", "チーム保存設定")}
					</DialogTitle>
					<DialogContent>
						<TextField
							autoFocus
							...
							inputProps={{
								"aria-label": t("TeamTimeline.team set name", "チーム名"),
							}}
							sx={{ mt: "4px" }}
						/>
							...
							<FormControlLabel
								control={
									<Checkbox
										...
										onChange={(event) =>
											setSaveCookingDraft(event.target.checked)
										}
									/>
								}
							...
							<FormControlLabel
								control={
									<Checkbox
										...
										onChange={(event) =>
											setSaveFieldDraft(event.target.checked)
										}
									/>
								}
								label={t(
									"TeamTimeline.team set save option field",
									"フィールド",
								)}
							/>
						</Box>
