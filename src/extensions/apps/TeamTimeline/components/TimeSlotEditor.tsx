...
import {
	getMealType,
	type MealType,
	type SleepStateLabel,
	type TimeSlot,
} from "../types/TimeSlotTypes";
import { generateTimeOptions, sortTimeSlots } from "../utils/TimeSlotUtils";
...
export default function TimeSlotEditor({
	...
	const validationMessage = React.useMemo(() => {
		switch (validationError) {
			case "consecutiveWake":
				return t(
					"TeamTimeline.time slot validation consecutive wake",
					"起床が連続しています",
				);
			...
			case "tooManyWakeSleepPairs":
				return t(
					"TeamTimeline.time slot validation too many pairs",
					"起床、就寝は1日に2回までです。",
				);
			default:
				return null;
	...
	const getDraftSlot = (slotId: string): TimeSlot | undefined =>
		draftSlots.find((slot) => slot.id === slotId);
	...
	const handleDraftSleepStateChange = (
		slotId: string,
		newState: SleepStateLabel,
	) => {
		setDraftSlots((current) =>
			current.map((slot) =>
				slot.id === slotId ? { ...slot, sleepState: newState } : slot,
			),
		);
	};
	...
	return (
		<Box>
			...
			>
				...
				<Button size="small" startIcon={<RestoreIcon />} onClick={onReset}>
