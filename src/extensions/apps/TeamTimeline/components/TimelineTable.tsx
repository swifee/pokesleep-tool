...
const TimelineTable = React.memo(
	({
		...
		const [swapDragSource, setSwapDragSource] =
			useState<SwapCellCoordinate | null>(null);
		const [swapDragTarget, setSwapDragTarget] =
			useState<SwapCellCoordinate | null>(null);
		const [activeSwapPointerId, setActiveSwapPointerId] = useState<
			number | null
		>(null);
		const [activeSwapPointerType, setActiveSwapPointerType] = useState<
			string | null
		>(null);
		const [swapDragPreview, setSwapDragPreview] =
			useState<SwapDragPreview | null>(null);
		...
		const slotOrderById = useMemo(() => {
			const entries = expandedTimeline.baseDaySlots.map(
				(baseSlot, index) => [baseSlot.id, index] as const,
			);
			return new Map<string, number>(entries);
		}, [expandedTimeline.baseDaySlots]);

		const dayBandBySlotId = useMemo(() => {
			const entries = expandedTimeline.dayBands.map(
				(band) => [band.afterDisplaySlotId, band.dayNumber] as const,
			);
			return new Map<string, number>(entries);
		...
		const dayEndEpByDayBandNumber = useMemo(() => {
			...
			for (const expandedSlot of expandedTimeline.expandedSlots) {
				const slotResults =
					result.slotResults.get(expandedSlot.slot.id) ||
				...
				for (const slotResult of slotResults) {
					...
					if (!shouldUseCookingTotals) {
						const allIngredients =
							...
							slotResult.skillIngredients.length > 0
				...
				if (shouldUseCookingTotals) {
					cumulativeEp +=
						cookingEpBySlotId.get(expandedSlot.slot.id) ??
		...
		const updateSwapDragTarget = useCallback(
			(nextTarget: SwapCellCoordinate | null) => {
		...
		const flushDragGhostPosition = useCallback(() => {
			...
			const translateY =
				preview.y -
				preview.pointerOffsetY -
				(swapDragLiftedRef.current ? dragLiftDistancePx : 0);
		...
		const resolveSwapCellFromPoint = useCallback(
			(clientX: number, clientY: number): SwapCellCoordinate | null => {
				const pointedElement = document.elementFromPoint(clientX, clientY);
				const swapCell = pointedElement?.closest(
					'[data-swap-drop-enabled="true"]',
				);
		...
		const handleSwapLongPressStart = useCallback(
			(detail: SwapLongPressStartDetail) => {
				...
				const initialCandidate = resolveSwapCellFromPoint(
					detail.clientX,
					detail.clientY,
				);
				const initialTarget = isSameSwapCell(initialCandidate, sourceCell)
					? null
					: initialCandidate;
				...
				const preview: SwapDragPreview = {
					...
					y: detail.clientY,
					label:
						detail.swappedPokemonName ??
						t("TeamTimeline.swap moving", "移動中"),
		...
		useEffect(() => {
			...
			const updateDragPosition = (clientX: number, clientY: number) => {
				...
				const nextTarget = isSameSwapCell(targetCandidate, swapDragSource)
					? null
					: targetCandidate;
		...
		const renderHeaderSlotContent = (
			pokemon: PokemonBoxItem | null,
		): React.ReactNode => {
			if (!pokemon) {
				return (
					<EmptySlot>
						...
						<span className="icon-placeholder">
							<AddIcon
								data-testid="timeline-header-empty-plus-icon"
		...
		const renderDayBandLabel = (dayNumber: number): React.ReactNode => {
			const dayLabel = t("TeamTimeline.day label", "{{day}}日目", {
				day: dayNumber,
			});
				...
				<DayBandSummaryCell>
					{dayNumber > 1 && shouldShowDayEndEp && (
						<>
							...
							<EpValue
								value={(
									dayEndEpByDayBandNumber.get(dayNumber) ?? 0
								).toLocaleString()}
							/>
						</>
					)}
		...
		return (
			...
			>
				<HeaderRow $fitToViewport={fitToViewport}>
					<CornerHeaderCell
						$fitToViewport={fitToViewport}
						data-testid="timeline-corner-header-cell"
					>
						{onOpenTimeSlotSettings && (
							<CornerHeaderButton
								type="button"
				...
				<DataSection>
					{shouldShowFirstDayBand && renderDayBandRow(1)}
					{expandedTimeline.expandedSlots.map((expandedSlot) => {
						const isFirstTimelineSlot =
							expandedSlot.dayIndex === 0 && expandedSlot.slotIndexInDay === 0;
						const slotResults =
							result.slotResults.get(expandedSlot.slot.id) ||
							result.slotResults.get(expandedSlot.originalSlotId) ||
							[];
							...
							<React.Fragment key={expandedSlot.slot.id}>
								...
								{result.cookingResult &&
									...
									(() => {
										...
										if (
											!cookingEvent ||
											(cookingEvent.recipeName == null &&
												cookingEvent.cookingEP === 0)
										) {
											return null;
										}
										return (
								...
								{dayBandNumber !== undefined && renderDayBandRow(dayBandNumber)}
							</React.Fragment>
						);
					})}
				</DataSection>
						...
						<SwapDragGhost
							...
							style={{
								...
								transform: `translate3d(${
									(swapDragPreviewRef.current ?? swapDragPreview).x -
									(swapDragPreviewRef.current ?? swapDragPreview).pointerOffsetX
								}px, ${
									(swapDragPreviewRef.current ?? swapDragPreview).y -
									(swapDragPreviewRef.current ?? swapDragPreview)
										.pointerOffsetY -
									(swapDragLifted ? dragLiftDistancePx : 0)
...
const TableContainer = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
...
const HeaderRow = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
		display: "flex",
		alignItems: "stretch",
		width: $fitToViewport ? "100%" : "max-content",
...
const CornerHeaderCell = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
...
const PokemonHeaderCell = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
		width: $fitToViewport
...
const DayBandRow = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
		margin: "0 0 1px",
		width: $fitToViewport ? "100%" : "max-content",
		minWidth: $fitToViewport ? "0" : "540px",
...
const DayBandLabelCell = styled("div")<{ $fitToViewport: boolean }>(
	({ $fitToViewport }) => ({
		width: $fitToViewport ? "var(--timeline-time-cell-width, 40px)" : "40px",
		minWidth: $fitToViewport ? "var(--timeline-time-cell-width, 40px)" : "40px",
		flexShrink: 0,
