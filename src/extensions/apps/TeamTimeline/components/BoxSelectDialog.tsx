...
} from "../../../../ui/IvCalc/PokemonFilterFooter";
...
function isValidIngredientSortType(
	value: unknown,
): value is IngredientName | "strength" | "count" {
	return (
		value === "strength" ||
		value === "count" ||
		(typeof value === "string" &&
			IngredientNames.includes(value as IngredientName))
	);
}

function isValidMainSkillSortType(
	value: unknown,
): value is MainSkillName | "strength" | "count" {
	return (
		value === "strength" ||
		value === "count" ||
		(typeof value === "string" &&
			MainSkillNames.includes(value as MainSkillName))
	);
}
...
function normalizeSortConfig(value?: Partial<BoxSortConfig>): BoxSortConfig {
	...
	return {
		...
		ingredient: isValidIngredientSortType(value?.ingredient)
			? value.ingredient
			: fallback.ingredient,
		mainSkill: isValidMainSkillSortType(value?.mainSkill)
			? value.mainSkill
			: fallback.mainSkill,
		descending:
			typeof value?.descending === "boolean"
				...
				: fallback.descending,
		warnItems:
			typeof value?.warnItems === "number"
				? value.warnItems
				: fallback.warnItems,
		warnDate:
			typeof value?.warnDate === "string" ? value.warnDate : fallback.warnDate,
	};
}
...
const BoxSelectDialog = React.memo(
	({ open, box, onSelect, onClose, onSelectNone }: BoxSelectDialogProps) => {
		...
		const [sortConfig, setSortConfig] = React.useState<BoxSortConfig>(
			initialConfigRef.current.sortConfig,
		);
		const [filterConfig, setFilterConfig] = React.useState<BoxFilterConfig>(
			initialConfigRef.current.filterConfig,
		);
		const [parameter, setParameter] = React.useState<StrengthParameter>(
			initialConfigRef.current.parameter,
		);
		...
		const onFilterFooterChange = React.useCallback(
