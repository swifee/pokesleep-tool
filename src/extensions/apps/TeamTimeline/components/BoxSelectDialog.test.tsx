...
vi.mock("../../../../ui/IvCalc/PokemonFilterFooter", () => ({
	default: ({
		...
		<div>
			...
			<button
				type="button"
				onClick={() => onChange({ ...value, sort: "name" })}
			>
				sort-name
			</button>
			<button
	...
	default: ({
		...
		open ? (
			<button
				type="button"
				onClick={() =>
					onChange(new BoxFilterConfig({ ...value, name: "alpha" }))
				}
			>
				apply-alpha-filter
			</button>
...
function getRenderedNames(): string[] {
	return screen
		.getAllByTestId("team-timeline-box-item-name")
...
describe("BoxSelectDialog", () => {
	...
	it("supports sorting and filtering like IvCalc box", () => {
		...
		render(
			<BoxSelectDialog open box={box} onSelect={vi.fn()} onClose={vi.fn()} />,
		);
	...
	it('shows only "swap none" special option in swap dialog', () => {
		const box = new PokemonBox([createPokemon("Pikachu", 10, "Bravo")]);
		...
		expect(
			screen.getByRole("button", { name: "TeamTimeline.swap none" }),
		).toBeDefined();
		expect(
			screen.queryByRole("button", { name: "TeamTimeline.swap no change" }),
		).toBeNull();
	});
});
