...
describe("TimeSlotEditor validation message", () => {
	it("shows consecutive wake message above add button", () => {
		...
		expect(
			(message.compareDocumentPosition(addButton) &
...
describe("TimeSlotEditor draft row", () => {
	it("adds unset draft row at bottom and registers only when time is selected", () => {
		...
		expect(
			hasColorStyle(
				draftRows[0].getAttribute("style"),
				"#e7f0ff",
				"231, 240, 255",
			),
		).toBe(true);
...
describe("TimeSlotEditor row move animation", () => {
		...
		vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockImplementation(
			function mockRect(this: HTMLElement) {
				if (this.dataset.rowKey) {
					...
					const rows = parent
						? Array.from(parent.querySelectorAll("[data-row-key]"))
						: [];
	...
	it("temporarily highlights moved rows after time edit", () => {
		...
		const secondRowTimeSelect = within(savedRowsBefore[1]).getAllByRole(
			"combobox",
		)[0];
		...
		expect(
			rowsAfterHighlight.some((row) =>
				hasColorStyle(row.getAttribute("style"), "#f5f5f5", "245, 245, 245"),
			),
		).toBe(true);
	});
