...
function createIngredientUsage(
	name: "apple" | "milk" | "honey",
	count: number,
) {
	return {
		name,
...
describe("TeamSummaryRow", () => {
	...
	it("renders EP panel before ingredient metadata", () => {
		...
		const content = container.textContent ?? "";
		expect(content.indexOf("total 6,000 EP")).toBeLessThan(
			content.indexOf("食材合計: 22"),
		);
	});
	...
	it("shows EP in berry -> skill -> ingredient order when cooking simulation is off", () => {
		...
		const ingredientItem = screen.getByTestId(
			"team-summary-ep-item-ingredient",
		);
		expect(
			berryItem.compareDocumentPosition(skillItem) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).not.toBe(0);
		expect(
			skillItem.compareDocumentPosition(ingredientItem) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).not.toBe(0);
	});
	...
	it("shows EP in berry -> skill -> cooking order when cooking simulation is on", () => {
		...
		expect(
			skillItem.compareDocumentPosition(cookingItem) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).not.toBe(0);
	});
	...
	it("shows 食材合計 as integer", () => {
		...
		expect(
			screen.getByText("食材合計:").parentElement?.textContent ?? "",
		).toContain("1,945");
	});
	...
	it("switches to daily average values when valueMode is dailyAverage", () => {
		...
		expect(
			screen.getByTestId("team-summary-ep-item-berry").textContent,
		).toContain("1,000 EP");
		expect(
			screen.getByTestId("team-summary-ep-item-ingredient").textContent,
		).toContain("667 EP");
		expect(
			screen.getByTestId("team-summary-ep-item-skill").textContent,
		).toContain("334 EP");
	...
	it("renders remaining pot capacity in cooking details as integer when floating-point noise exists", () => {
		const firstCookingEvent = COOKING_RESULT.dailySummaries[0]?.events[0];
		if (!firstCookingEvent) {
			throw new Error("Expected base cooking event fixture to exist.");
		}
	...
	it("renders average cooking summary in requested format and groups recipes below 1.0 count", () => {
		...
		expect(content).toContain(
			"translated:TeamTimeline.recipe highBaseVisibleRecipe : 平均9,800 EP × 2回",
		);
		expect(content).toContain(
			"translated:TeamTimeline.recipe lowBaseVisibleRecipe : 平均4,200 EP × 1.5回",
		);
		expect(
			content.indexOf("translated:TeamTimeline.recipe highBaseVisibleRecipe"),
		).toBeLessThan(
			content.indexOf("translated:TeamTimeline.recipe lowBaseVisibleRecipe"),
		);
		...
		expect(popoverText).toContain(
			"translated:TeamTimeline.recipe groupedRecipe : 平均550 EP × 0.06回",
		);
		expect(popoverText).not.toContain(
			"translated:TeamTimeline.recipe excludedRecipe",
		);
	});
	...
	it("shows leftover include toggle after leftover ingredient block when enabled", () => {
		...
		const toggle = screen.getByTestId(
			"leftover-extra-usage-toggle-details",
		) as HTMLInputElement;
		expect(toggle.checked).toBe(false);
		const content = document.body.textContent ?? "";
		expect(content).toContain("追加食材使用分を含む");
		expect(content.indexOf("あまり食材")).toBeLessThan(
			content.indexOf("追加食材使用分を含む"),
		);
	});
