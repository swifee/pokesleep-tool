...
const TimelineBonusSettingsPanel = React.memo(
	({
		...
		const applyParameter = React.useCallback(
			(next: StrengthParameter) => {
				onSettingsChange(strengthParameterToTimelineBonusSettings(next));
			},
			[onSettingsChange],
		);

		const handleSyncChange = React.useCallback(
		...
		const handleRecipeBonusChange = React.useCallback(
		...
		const handleRecipeLevelChange = React.useCallback(
		...
		return (
			<Container>
				<FormControlLabel
					control={
						<Switch checked={syncWithIvParameter} onChange={handleSyncChange} />
					}
					label={t(
						"TeamTimeline.sync with iv settings",
						"個体値計算機の設定と連動",
					)}
				/>
				...
				<AreaSection>
					<AreaControlGroup value={parameter} onChange={applyParameter} />
				</AreaSection>
				...
				<SectionRow>
					<span>{t("average recipe level")}:</span>
					<SliderWrap>
						<Slider
				...
				{cookingSimEnabled && (
					...
					>
						{t(
							"TeamTimeline.cooking recipe bonus and average level note",
							"※[レシピボーナス]と[平均レシピレベル]は料理シミュレーションOFF時にのみ適用されます",
						)}
					</Typography>
				)}
