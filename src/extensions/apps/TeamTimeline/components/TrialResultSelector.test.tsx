...
describe("TrialResultSelector", () => {
	...
	it("renders status text and prev/next navigation", () => {
		...
		expect(screen.getByTestId("trial-status-row").textContent).toContain(
			"(シード: 2)",
		);

		const statusRow = screen.getByTestId("trial-status-row");
		expect(
			within(statusRow).queryByRole("button", { name: "previous-trial" }),
		).toBeNull();
		expect(
			within(statusRow).queryByRole("button", { name: "next-trial" }),
		).toBeNull();

		const sliderControls = screen.getByTestId("trial-slider-controls");
		const prevButton = within(sliderControls).getByRole("button", {
			name: "previous-trial",
		});
		const nextButton = within(sliderControls).getByRole("button", {
			name: "next-trial",
		});
	...
	it("updates highlighted histogram bin while slider is moving", () => {
		...
		expect(
			screen
				.getByTestId("trial-distribution-bar-1")
		...
		expect(
			screen
				.getByTestId("trial-distribution-bar-2")
				.getAttribute("data-active"),
		).toBe("true");
	});
	...
	it("changes histogram bin count based on trial count", () => {
		...
		expect(
			chart100.querySelectorAll('[data-testid^="trial-distribution-bar-"]')
				.length,
		).toBe(16);

		rerender(
			<TrialResultSelector
				results={createResults(1000)}
				selectedIndex={0}
				onSelect={vi.fn()}
			/>,
		);
		...
		expect(
			chart1000.querySelectorAll('[data-testid^="trial-distribution-bar-"]')
				.length,
		).toBe(51);
	});
