    ...
    return [
        {
            ...
            swaps: [
                { dayIndex: 0, slotId: 'slot-1', teamSlotIndex: 0, newPokemonId: 999, initialEnergy: 80 },
                { dayIndex: 1, slotId: 'slot-1', teamSlotIndex: 0, newPokemonId: 999, initialEnergy: 80 },
                { dayIndex: 2, slotId: 'slot-2', teamSlotIndex: 2, newPokemonId: SWAP_NONE_POKEMON_ID, initialEnergy: 0 },
            ],
            noCollectCells: [],
        ...
        },
        {
            ...
            swaps: [
                { dayIndex: 0, slotId: 'slot-1', teamSlotIndex: 0, newPokemonId: 200, initialEnergy: 90 },
                { dayIndex: 0, slotId: 'slot-3', teamSlotIndex: 1, newPokemonId: 201, initialEnergy: 90 },
            ],
            noCollectCells: [],
            lastSimulationSnapshot: {
                averageTotalEP: 7777,
        ...
        },
        {
            ...
            team: [null, null, null, null, null],
            swaps: [
                { dayIndex: 0, slotId: 'slot-1', teamSlotIndex: 0, newPokemonId: SWAP_NONE_POKEMON_ID, initialEnergy: 0 },
            ],
            noCollectCells: [],
...
describe('TeamSetToolbar', () => {
    ...
    it('renders menu details and fires select/duplicate/delete/create handlers', () => {
        ...
        fireEvent.mouseDown(screen.getByRole('combobox', { name: 'チームセット選択' }));
        ...
        expect(within(firstItem).getByTestId('team-set-icon-0-0').getAttribute('style') ?? '').toContain('border-radius: 4px');

        const secondItem = screen.getByTestId('team-set-menu-item-1');
        expect(within(secondItem).getByText('+2')).toBeDefined();
        const secondItemEmptySlot = within(secondItem).getByTestId('team-set-empty-slot-1-0');
        expect(secondItemEmptySlot.getAttribute('style') ?? '').toContain('display: block');
        expect(secondItemEmptySlot.getAttribute('style') ?? '').toContain('border-radius: 4px');
        expect(within(firstItem).getByTestId('team-set-average-ep-0').textContent).toBe('12,345 EP');
        expect(within(firstItem).getByTestId('team-set-average-ep-0').getAttribute('data-stale')).toBe('false');
        expect(within(secondItem).getByTestId('team-set-average-ep-1').textContent).toBe('7,777 EP');
        expect(within(secondItem).getByTestId('team-set-average-ep-1').getAttribute('data-stale')).toBe('true');

        const thirdItem = screen.getByTestId('team-set-menu-item-2');
        expect(within(thirdItem).queryByText('+0')).toBeNull();
        expect(within(thirdItem).getByTestId('team-set-average-ep-2').textContent).toBe('');

        fireEvent.click(within(firstItem).getByTestId('team-set-duplicate-button-0'));
