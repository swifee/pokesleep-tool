...
describe("CookingResultRow", () => {
	...
	it("shows 0 (x) for ingredients that would remain only without extra ingredients", () => {
		const event: CookingEventResult = {
			...EVENT,
			bagIngredientsBeforeCooking: [],
			bagIngredientsBeforeCookingWithoutExtra: [{ name: "apple", count: 5 }],
		};
	...
	it("opens help popover when clicking question mark icon", () => {
		...
		expect(
			screen.getByText("カッコ内はここまで追加食材を一切入れなかった場合の数"),
		).toBeDefined();
	});
	...
	it("hides used ingredients and remaining pot info in simple mode", () => {
		...
		render(
			<CookingResultRow event={event} teamSize={5} displayMode="simple" />,
		);
