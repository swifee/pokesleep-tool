...
import type { IngredientResult, TimeSlotResult } from "../types/TimeSlotTypes";
import {
...
function countPreviousMatchingItems<T>(
	items: readonly T[],
	currentIndex: number,
	matcher: (candidate: T) => boolean,
): number {
	return items.slice(0, currentIndex).filter(matcher).length;
}
...
function buildOccurrenceKey(
	...
	return [prefix, ...parts, occurrence].join("-");
}
...
function buildIngredientKey(
	...
	const occurrence = countPreviousMatchingItems(
		ingredients,
		currentIndex,
		(candidate) =>
...
const TimelineCell = React.memo((props: TimelineCellProps) => {
	...
	const removeSwapButtonTitle = t(
		"TeamTimeline.swap remove",
		"入れ替え設定を解除",
	);
	const noCollectButtonTitle = t(
		"TeamTimeline.no collect toggle",
		"回収しない",
	);
	...
	const showNoCollectButton =
		!disableSwapUi && !hasSwapInfo && !hasSwap && Boolean(onNoCollectToggle);
	...
	const isCompactLayout =
		isCompactEmptyCell || compactFirstSlot || isSimpleMode;
	...
	const handleSwapRemoveButtonClick = (
		event: React.MouseEvent<HTMLButtonElement>,
	) => {
		event.stopPropagation();
		if (suppressNextActionClickRef.current) {
	...
	const handleNoCollectButtonClick = (
		event: React.MouseEvent<HTMLButtonElement>,
	) => {
		event.stopPropagation();
		onNoCollectToggle?.();
	...
	const handleSwapInfoPointerDown = (
	...
	const handleSwapInfoPointerMove = (
		event: React.PointerEvent<HTMLButtonElement>,
	) => {
		if (
			dragPointerIdRef.current !== event.pointerId ||
	...
	const handleSwapInfoContextMenu = (
		event: React.MouseEvent<HTMLButtonElement>,
	) => {
	...
	const handleSwapInfoPointerCancel = (
		event: React.PointerEvent<HTMLButtonElement>,
	) => {
		if (dragPointerIdRef.current !== event.pointerId) {
			return;
		...
		return (
			...
			>
				<SwapInfoMainButton
					...
					onPointerDown={
						isLongPressEnabled ? handleSwapInfoPointerDown : undefined
					}
					onPointerMove={
						isLongPressEnabled ? handleSwapInfoPointerMove : undefined
					}
					onPointerUp={isLongPressEnabled ? handleSwapInfoPointerUp : undefined}
					onPointerCancel={
						isLongPressEnabled ? handleSwapInfoPointerCancel : undefined
					}
					onContextMenu={
						isLongPressEnabled ? handleSwapInfoContextMenu : undefined
					}
					title={swapButtonTitle}
				>
				...
				{onRemoveSwapClick && (
					<SwapRemoveButton
						...
						onPointerUp={
							isLongPressEnabled ? handleSwapInfoPointerUp : undefined
						}
						onPointerCancel={
							isLongPressEnabled ? handleSwapInfoPointerCancel : undefined
						}
			...
			return (
				...
				>
					{Array.from(
						...
						(_, iconIndex) => `skill-prefix-${iconIndex + 1}`,
					).map((iconKey) => (
						<TeamTimelineIcon
							key={iconKey}
							name="skill"
	...
	const renderTextWithHealIcon = React.useCallback(
	...
	const buildProxyDetailParts = (
		event: NonNullable<TimeSlotResult["proxySkillEvents"]>[number],
	): string[] => {
		const parts: string[] = [];
		const isMetronomeStockpile =
			event.source === "metronome" &&
			event.resolvedSkillName === "Charge Strength S (Stockpile)";
		if ((event.selfEnergyRecovery ?? 0) > 0) {
			parts.push(`❇️+${Math.round(event.selfEnergyRecovery ?? 0)}`);
		}
		if (
			(event.teamEnergyRecoveryPerMember ?? 0) > 0 &&
			(event.teamEnergyRecoveryTargetCount ?? 0) > 0
		) {
			parts.push(
				`❇️+${Math.round(event.teamEnergyRecoveryPerMember ?? 0)}(ALL)`,
			);
		}
		if ((event.stockpileStoreCount ?? 0) > 0 && !isMetronomeStockpile) {
			parts.push(
				`たくわえる(${event.stockpileCountAtStore ?? event.stockpileStoreCount})`,
			);
		}
		...
		if ((event.supportSkillBerryEP ?? 0) > 0) {
			parts.push(
				`+${Math.round(event.supportSkillBerryEP ?? 0).toLocaleString()} EP`,
			);
		} else if ((event.directEP ?? 0) > 0) {
			parts.push(`+${Math.round(event.directEP ?? 0).toLocaleString()} EP`);
		}
		const totalGreatSuccessCount =
			(event.berryBurstGreatSuccessCount ?? 0) +
			(event.ingredientDrawGreatSuccessCount ?? 0);
		...
		if ((event.tastyChanceIncreasePercent ?? 0) > 0) {
			parts.push(
				`料理大成功+${Math.round(event.tastyChanceIncreasePercent ?? 0)}%`,
			);
		}
		if ((event.dreamShardCount ?? 0) > 0) {
			parts.push(
				`ゆめのかけら+${Math.round(event.dreamShardCount ?? 0).toLocaleString()}`,
			);
		}
		return parts;
	};
	...
	if (result === null) {
		return (
			<StyledCell
				...
				data-swap-drop-enabled={
					!disableSwapUi && swapSlotId !== undefined && dayIndex !== undefined
						? "true"
						: "false"
				}
			>
				<EmptyContent>{renderPokemonIcon()}</EmptyContent>
				{renderSwapInfo()}
	...
	const totalSkillRecoveryInEnergyLine = Math.round(
		result.skillRecovery + result.selfSkillRecovery,
	);
	const energyBarWidth = Math.max(
		0,
		Math.min(100, (result.energyEnd / 150) * 100),
	);
	...
	const slotIngredients = sortIngredientsByCountDesc(
		result.ingredients.filter((i) => i.count > 0),
	);
	const overflowIngredients = sortIngredientsByCountDesc(
		result.overflowIngredients.filter((i) => i.count > 0),
	);
	const skillIngredients = sortIngredientsByCountDesc(
		(result.skillIngredients ?? []).filter((i) => i.count > 0),
	);
	...
	const teamEnergyRecoveryGivenPerMember =
		result.teamEnergyRecoveryGivenPerMember ?? 0;
	const teamEnergyRecoveryGivenTargetCount =
		result.teamEnergyRecoveryGivenTargetCount ?? 0;
	const hasEventStyleSkill =
		...
		hasProxyEventStyle;
	const totalGreatSuccessCount =
		(result.berryBurstGreatSuccessCount ?? 0) +
		(result.ingredientDrawGreatSuccessCount ?? 0);
	const simpleCookingCount = result.ingredients.reduce(
	...
	if (
		teamEnergyRecoveryGivenPerMember > 0 &&
		teamEnergyRecoveryGivenTargetCount > 0
	) {
		aggregateDetailParts.push(
			`❇️+${Math.round(teamEnergyRecoveryGivenPerMember)}(ALL)`,
		);
	}
	if (result.stockpileStoreCount > 0) {
		aggregateDetailParts.push(
			`たくわえる(${result.stockpileCountAtStore ?? result.stockpileStoreCount})`,
		);
	}
	if (result.stockpileSpitCount > 0) {
		aggregateDetailParts.push(
			`はきだす(${result.stockpileCountAtSpit ?? result.stockpileSpitCount})`,
		);
	}
	...
	if (isHelperBoostOnlySupportEvents && result.supportSkillBerryEP > 0) {
		aggregateDetailParts.push(
			`+${Math.round(result.supportSkillBerryEP).toLocaleString()} EP`,
		);
	} else if (
		result.directSkillEP > 0 &&
		result.supportHelpEvents.length === 0
	) {
	...
	if ((result.cookingPotCapacityIncrease ?? 0) > 0) {
		aggregateDetailParts.push(
			`鍋容量+${Math.round(result.cookingPotCapacityIncrease ?? 0)}`,
		);
	}
	const tastyChanceDetail =
		(result.tastyChanceIncreasePercent ?? 0) > 0
			? `料理大成功+${Math.round(result.tastyChanceIncreasePercent ?? 0)}%`
			: null;
	if ((result.dreamShardCount ?? 0) > 0) {
		aggregateDetailParts.push(
			`ゆめのかけら+${Math.round(result.dreamShardCount ?? 0).toLocaleString()}`,
		);
	}
	...
	if (hasEventStyleSkill) {
		if (!hasProxyEventStyle) {
				...
				const eventKey = buildOccurrenceKey(
					...
					countPreviousMatchingItems(
						...
						(candidate) =>
							...
							candidate.recovery === event.recovery &&
			...
			moonlightEvents.forEach((event, index) => {
				const eventKey = buildOccurrenceKey(
					"moonlight",
					[event.targetPokemonId, event.recovery],
					countPreviousMatchingItems(
						moonlightEvents,
						index,
						(candidate) =>
				...
				const eventKey = buildOccurrenceKey(
					...
					countPreviousMatchingItems(
						...
						(candidate) =>
							candidate.source === event.source &&
							candidate.targetPokemonId === event.targetPokemonId &&
							candidate.helpCount === event.helpCount &&
							candidate.berryCount === event.berryCount &&
							candidate.berryEP === event.berryEP,
					),
				);
				skillDetailLines.push(
					<SkillDetailLine key={eventKey}>
		...
		proxySkillEvents.forEach((event, index) => {
			...
			const eventKey = buildOccurrenceKey(
				...
				[
					event.source,
					event.triggeredSkillName,
					event.resolvedSkillName,
					event.resolvedSkillLevel,
					event.copiedFromPokemonId ?? "none",
				],
				countPreviousMatchingItems(
					proxySkillEvents,
					index,
					(candidate) =>
						candidate.source === event.source &&
						candidate.triggeredSkillName === event.triggeredSkillName &&
						candidate.resolvedSkillName === event.resolvedSkillName &&
						candidate.resolvedSkillLevel === event.resolvedSkillLevel &&
						candidate.copiedFromPokemonId === event.copiedFromPokemonId,
				),
			);
			skillDetailLines.push(
				<SkillDetailLine key={eventKey}>
					...
					{sortIngredientsByCountDesc(
						(event.skillIngredients ?? []).filter(
							(ingredient) => ingredient.count > 0,
						),
					).map((ingredient, ingredientIndex, ingredients) => (
						<React.Fragment
							key={buildIngredientKey(
								`${eventKey}-ingredient`,
								ingredients,
		...
		if (isHelperBoostOnlySupportEvents) {
			const triggerCount =
				result.skillTriggerCount > 0 ? result.skillTriggerCount : 1;
			skillDetailLines.push(
				<SkillDetailLine key="helper-boost-aggregate">
					<SkillPrefixIcons
						count={triggerCount}
						testId="timeline-cell-skill-prefix-helper-boost"
					/>
					{aggregateDetailParts.length > 0 &&
						renderTextWithHealIcon(
							` ${aggregateDetailParts.join(" ")}`,
							"helper-boost-aggregate",
						)}
					{skillIngredients.map((ingredient, ingredientIndex) => (
						<React.Fragment
							key={buildIngredientKey(
								"helper-boost-ingredient",
								skillIngredients,
								ingredientIndex,
							)}
						>
			...
			if (skillIngredients.length > 0) {
				...
				skillDetailLines.push(
					<SkillDetailLine key="skill-ingredients-aggregate">
						<SkillPrefixIcons
							count={triggerCount}
							testId="timeline-cell-skill-prefix-ingredients"
						/>
						{skillIngredients.map((ingredient, ingredientIndex) => (
							<React.Fragment
								key={buildIngredientKey(
									"skill-ingredients-aggregate",
									skillIngredients,
									ingredientIndex,
								)}
							>
			...
			if (aggregateDetailParts.length > 0) {
				...
				skillDetailLines.push(
					<SkillDetailLine key="aggregate">
						<SkillPrefixIcons
							count={triggerCount}
							testId="timeline-cell-skill-prefix-aggregate"
						/>
						{aggregateDetailParts.length > 0
							? renderTextWithHealIcon(
									` ${aggregateDetailParts.join(" ")}`,
	...
	return (
		...
		>
			<TopEnergyArea>
				...
				<EnergySummary>
					{/* Line 1: Energy Value */}
					<EnergyLine>げんき{Math.round(result.energyEnd)}</EnergyLine>
					<EnergyBarTrack>
						<EnergyBarFill
							data-testid="timeline-cell-energy-bar-fill"
							style={{
				...
				<RecoveryInfoLine>
					...
					{result.wakeRecovery > 0 && (
						<>
							<TeamTimelineIcon
								name="sleep"
								data-testid="timeline-cell-recovery-icon-sleep"
							/>
							+{result.wakeRecovery}
						</>
					)}
					...
					{result.mealRecovery > 0 && (
						<>
							<TeamTimelineIcon
								name="cooking"
								data-testid="timeline-cell-recovery-icon-cooking"
							/>
							+{result.mealRecovery}
						</>
					)}
					...
					{totalSkillRecoveryInEnergyLine > 0 && (
						<>
							<TeamTimelineIcon
								name="heal"
								data-testid="timeline-cell-recovery-icon-heal"
								data-heal-icon="true"
							/>
					...
					{totalSkillRecoveryInEnergyLine > 0 &&
						...
						" "}
					{result.badDreamsDamageTaken > 0 &&
						`🖤-${Math.round(result.badDreamsDamageTaken)}`}
				</RecoveryInfoLine>
			)}
			...
			<ResourceLine>
				{!(isFirstSlot && result.helpCount === 0) && (
					<HelpLine>
						<TeamTimelineIcon
							name="work"
							data-testid="timeline-cell-help-icon-work"
						/>
						{result.helpCount}
					</HelpLine>
				)}
				...
				<SkillLine>
					...
					{result.skillOverflowCount > 0 && (
						<SkillOverflowLine>
							<TeamTimelineIcon
								name="skill_none"
								data-testid="timeline-cell-skill-overflow-icon"
							/>
						</SkillOverflowLine>
					)}
				</SkillLine>
...
const SwapInfoContainer = styled("div")<{ $dragState: SwapDragState }>(
	({ $dragState }) => ({
