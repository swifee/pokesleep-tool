...
import type { DailySummary } from "../types/TimeSlotTypes";
...
describe("DailySummaryRow", () => {
	it("renders all summaries when there are more than 5 pokemon", () => {
		...
		const summaries = items.map((item, index) =>
			createDailySummary(item.id, index + 1),
		);
		...
		expect(
			container.querySelectorAll('[data-testid="daily-summary-row"]'),
		).toHaveLength(1);
		expect(
			container.querySelectorAll('[data-testid="daily-summary-cell"]'),
		).toHaveLength(6);
	});
	...
	it("uses responsive grid columns so card width follows available columns", () => {
		...
		expect(gridStyle.gridTemplateColumns.replace(/\s+/g, "")).toContain(
	...
	it("renders EP box between pokemon name and other stats", () => {
		...
		const berryEpLine = screen.getByTestId(
			`daily-summary-ep-berry-${items[0].id}`,
		);
		const helpCountLine = screen.getByTestId(
			`daily-summary-count-help-${items[0].id}`,
		);
		expect(
			berryEpLine.compareDocumentPosition(helpCountLine) &
	...
	it("matches lower metrics left margin to EP box inset without changing right margin", () => {
		...
		const metricsArea = screen.getByTestId(
			`daily-summary-metrics-${items[0].id}`,
		);
		const berryEpLine = screen.getByTestId(
			`daily-summary-ep-berry-${items[0].id}`,
		);
	...
	it("applies dailyAverage conversion and rounds EP to integer", () => {
		...
		expect(
			screen.getByTestId(`daily-summary-count-help-${items[0].id}`).textContent,
		).toContain("2.5");
	...
	it("renders berry before skill in individual card EP and count lines", () => {
		...
		const skillEpLine = screen.getByTestId(
			`daily-summary-ep-skill-${items[0].id}`,
		);
		const berryCountLine = screen.getByTestId(
			`daily-summary-count-berry-${items[0].id}`,
		);
		const skillCountLine = screen.getByTestId(
			`daily-summary-count-skill-${items[0].id}`,
		);

		expect(
			berryEpLine.compareDocumentPosition(skillEpLine) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).not.toBe(0);
		expect(
			berryCountLine.compareDocumentPosition(skillCountLine) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).not.toBe(0);
	});
	...
	it("renders skill_none overflow icon with same size as skill icon and keeps overflow count text style", () => {
		...
		const skillLine = screen.getByTestId(
			`daily-summary-count-skill-${items[0].id}`,
		);
		const skillNoneIcon = screen.getByTestId(
			`daily-summary-count-icon-skill-overflow-${items[0].id}`,
		);
		const overflow = screen.getByTestId(
			`daily-summary-count-skill-overflow-${items[0].id}`,
		);
	...
	it("opens skill ingredient popover by clicking the skill count number", () => {
		...
		fireEvent.click(
			screen.getByTestId(`skill-ingredient-trigger-${items[0].id}`),
		);
