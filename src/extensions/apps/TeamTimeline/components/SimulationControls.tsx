...
export const SimulationControls: React.FC<SimulationControlsProps> = ({
	...
	const favoriteTypeLabels = React.useMemo(() => {
		...
		const favoriteTypes =
			fixedBerries.length === 3 ? fixedBerries : bonusSettings.favoriteType;
	...
	const renderCookingCategoryValue = (value: unknown) => {
		if (value === "curry") {
			return t(
				"TeamTimeline.simulation cooking category curry short",
				"カレー",
			);
		}
		if (value === "dessert") {
			return t(
				"TeamTimeline.simulation cooking category dessert short",
				"デザート",
			);
		}
	...
	return (
		<Box sx={CONTROLS_PANEL_STYLE} data-testid="simulation-controls-panel">
			...
			>
				...
				>
					<Typography
						sx={{ ...CONTROL_LABEL_STYLE, whiteSpace: "nowrap" }}
						data-testid="field-summary-text"
					>
						{fieldLabel}
					</Typography>
					<Typography
						sx={{ ...CONTROL_LABEL_STYLE, whiteSpace: "nowrap" }}
						data-testid="camp-ticket-summary-text"
					>
						{campTicketLabel}
					</Typography>
					<Typography
						sx={{ ...CONTROL_LABEL_STYLE, whiteSpace: "nowrap" }}
						data-testid="event-summary-text"
					>
						{eventLabel}
					</Typography>
				</Box>
				...
				<Button
					...
					sx={{
						...
						"&::before": {
							...
							transition:
								simulationLoading && clampedProgress > 0
									? "width 140ms linear"
									: "none",
							zIndex: 0,
						},
