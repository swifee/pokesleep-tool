...
import type { CookingSimulationResult } from "./CookingTypes";
		...
		sleepState:
			legacy.label === "sleep"
				...
				: legacy.label === "wake"
					? "wake"
					: "none",
		hasMeal: legacy.label
			? ["breakfast", "lunch", "dinner"].includes(legacy.label)
			: false,
	};
}
...
export function clampSimulationDays(days: number): number {
	...
	return Math.min(
		MAX_SIMULATION_DAYS,
		Math.max(MIN_SIMULATION_DAYS, Math.floor(days)),
	);
}
