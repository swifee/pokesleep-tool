...
import type { MealType } from "./TimeSlotTypes";
...
export interface LeftoverIngredients {
	...
	readonly byPokemon: ReadonlyMap<
		number,
		Readonly<Partial<Record<IngredientName, number>>>
	>;
...
export interface AverageCookingSummary {
	...
	/** 期間終了時の余り食材平均 */
	readonly leftoverIngredients: readonly {
		readonly name: IngredientName;
		readonly count: number;
	}[];
	/** 追加食材配分後の期間終了時あまり食材平均 */
	readonly leftoverIngredientsAfterExtra?: readonly {
		readonly name: IngredientName;
		readonly count: number;
	}[];
	/** 初期食材由来EP合計の試行平均 */
