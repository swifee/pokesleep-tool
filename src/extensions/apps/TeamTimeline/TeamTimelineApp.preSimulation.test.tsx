...
vi.mock("./components/TeamSetToolbar", () => ({
	default: ({
		...
		<div data-testid="team-set-toolbar">
			<button
				type="button"
				data-testid="team-set-create-click"
				onClick={() => onCreate?.()}
			>
				create-team-set
			</button>
			<button
				type="button"
				data-testid="team-set-select-second-click"
				onClick={() => onSelect?.(1)}
			>
				select-team-set-2
			</button>
...
describe("TeamTimelineApp pre-simulation timeline", () => {
	...
	it("shows timeline table before simulation and hides summary areas", () => {
		...
		expect(
			screen.getByTestId("team-timeline-pre-simulation-table"),
		).toBeDefined();
		expect(
			screen
				.getByTestId("team-timeline-pre-simulation-scroll-container")
		...
		expect(timeline.getAttribute("data-team")).toBe(
	...
	it("switches team state by team set dropdown selection", () => {
		...
		expect(screen.getByTestId("timeline-table").getAttribute("data-team")).toBe(
	...
	it("opens team box dialog when clicking timeline header slot", () => {
		...
		expect(
			screen.getByTestId("team-box-dialog").getAttribute("data-item-count"),
		).toBe("0");
		expect(
			screen.getByTestId("swap-box-dialog").getAttribute("data-item-count"),
		).toBe("0");
	...
	it("shows current sleep energy value next to slider in settings tab", () => {
		...
		expect(
			screen.getByTestId("team-timeline-sleep-energy-value").textContent,
		).toBe("50");
	});
	...
	it("restores saved cooking and field settings from active team set and when switching team set", async () => {
		...
		localStorage.setItem(
			"PstTeamTimelineCookingSettings",
			JSON.stringify({
				enabled: false,
				category: "curry",
				recipeLevels: {},
